...
A [mintaillesztés][pattern-matching] kifejező elágazó kódot tesz lehetővé, a [destrukturálás][destructuring] pedig elegáns módot ad arra, hogy az értékeket változókhoz kössük.

### Egyszerű mintaillesztés

A mintaillesztéssel a `case` utasítás segítségével értékeket köthetsz változókhoz, vagy a `_` helyettesítő karakterrel el is dobhatod őket.
...
A destrukturálás értékeket köthet `let` kötésekben, függvényargumentumokban, és természetesen a `case` kifejezésekben is, valahányszor az adatoknak csak egyetlen lehetséges formája van.
...
A destrukturálás [rekordoknál][records-pattern-matching] is használható, illetve az [`as` kulcsszóval][as-keyword] is.
