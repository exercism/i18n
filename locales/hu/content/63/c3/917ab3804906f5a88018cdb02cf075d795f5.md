...
A referenciák megértéséhez a tulajdonlást is meg kell értenünk. Minden értéknek egyetlen tulajdonosa van. Amikor a scope-jából kilépünk, az érték eldobódik.
Például:
...
A [referencia][borrow] egy tulajdonolt érték _kölcsönzését_ jelöli. Ahelyett, hogy átadnánk a `my_string` tulajdonlását, a `log` kölcsönözheti, majd a `log` végén visszaadhatja a `main`-ben a `my_string`-nek.

Ehhez a `msg`-t referenciaként definiáljuk úgy, hogy egy és-jelet írunk elé. A `&String` típust így olvassuk: „referencia egy `String`-re”.

Mivel a `log` argumentuma most már referencia, referenciát kell átadnunk. Ahhoz, hogy referenciát kapjunk a `my_string`-hez, írjunk elé egy és-jelet, ez a [kölcsönző operátor](https://doc.rust-lang.org/reference/expressions/operator-expr.html#borrow-operators). Ez kölcsönadja a `my_string`-et a `log`-nak, ameddig az a függvény fut.
...
## A referenciák alapértelmezés szerint módosíthatatlanok

Egy értékre egy adott pillanatban több referencia is mutathat, ameddig azok mind módosíthatatlanok, ahogy az alábbi példa mutatja
...
## A referenciák dereferálhatók

Néha egy referenciát dereferálni kell, hogy hozzáférjünk az értékéhez.
...
Az érték eléréséhez a dereferáló operátort (`*`) a referencia elé írjuk, ahogy az alábbi példa mutatja
...
## A referenciák lehetnek módosíthatók

A referenciák úgy tehetők módosíthatóvá, hogy az és-jel után `mut`-ot írunk, ahogy az alábbi példa mutatja
...
Egy értékre egy adott pillanatban vagy egyetlen módosítható referencia, vagy akárhány módosíthatatlan referencia mutathat, ahogy az alábbi példa mutatja
...
Ez azért működik, mert a fordító tudja, hogy a módosítható kölcsönzések nem fedik át egymást
