...
| ----- | ---------- | -------------------------------------------------- |
| Égal | `$a == $b` | vrai si `$a` est égal à `$b` après jonglage de types. |
| Identique | `$a === $b` | vrai si `$a` est égal à `$b` et du même type. |
| Différent | `$a != $b` | vrai si `$a` n'est pas égal à `$b` après jonglage de types. |
| Différent | `$a <> $b` | vrai si `$a` n'est pas égal à `$b` après jonglage de types. |
...
| Vaisseau spatial | `$a <=> $b` | renvoie un entier inférieur, égal ou supérieur à `0`i lorsque `$a`. |

PHP dispose de définitions distinctes qui distinguent `equal` de `identical`.
Parfois, `identical` est également appelé `strictly equal`.
