...
Comme on l'a vu dans le concept Fondamentaux, un programme AWK est composé de **paires motif-action**.
...
Le « motif » est n'importe quelle expression AWK.
Le fait que le résultat de l'expression soit vrai ou faux détermine si l'action est exécutée.
...
Utilise l'opérateur de correspondance `~` pour faire correspondre un champ donné.
...
C'est si courant qu'il existe une forme abrégée : on peut omettre `$0` et `~` et écrire simplement `/regex/`.
...
~~~~exercism/note
Compare ce one-liner AWK avec la commande grep équivalente
...
On approfondira la variante d'expressions régulières de GNU AWK dans un autre concept.
...
N'importe quelle fonction [prédéfinie][builtins] ou [définie par l'utilisateur][user-defined] peut être utilisée dans une expression, et donc dans le motif.
Quelques exemples :
...
Plus de détails se trouvent dans le manuel : [Éléments de motif][pattern-elements]
