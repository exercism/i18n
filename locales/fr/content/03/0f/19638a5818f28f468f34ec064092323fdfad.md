...
Décimal explore une alternative sans perte de précision pour les valeurs à virgule flottante. Cela met en évidence la différence fondamentale entre l'arithmétique à virgule flottante et la façon dont les humains interprètent les valeurs décimales. Un bon sujet à maîtriser pour les programmeurs débutants.

D'autres concepts utiles sont aussi abordés, comme les traits implémentés pour les types primitifs.

Les variantes d'approche sont très limitées, car la solution n'a pas besoin d'être complexe sur le plan algorithmique.

## Concepts extraits

Cela inclut tous les concepts montrés dans les exemples, ainsi que quelques concepts connexes qui ne sont pas appliqués ici.

**En gras : les éléments de niveau concept**
...
- **Itérateur**
  ...
  - `rev`
  - `position`
- Connexes, mais non appliqués ici : **génériques**
- **structures**
- **traits**
  - **Ordre**
  - **Égalité**
  - **Debug**
  - **Default**
  - `From`, `TryFrom` et `FromStr`
- **Traits numériques**
  - Add
  - Sub
  - Mul
  - Div
  - variantes `*Assign` de ce qui précède
  - Pow
    - `powf32`/`powif32` et `powf64`/`powif64`
  - **variantes saturating / overflowing / wrapping / checked de ce qui précède**
- **mot-clé `type`**
- **Dérivation**
- **Fonctions**
  - **Méthodes**
- **Syntaxe des méthodes** (`Type::` vs `self.`)
- **Visibilité**
- **Références**
  - Déréférencement
- **blocs `impl`**
- `new()` : pratique courante
...
- **variables**
  - **`let`/`static`/`const`**
  - Conversion de type
  - syntaxe d'affectation `let x = { //scope }`
...
- **Opérateur turbofish**
- **opérateur `?`**
- **`return` anticipé explicite**
- **syntaxe `use`**
  - utiliser plusieurs modules sur une seule ligne
- **Opt :** **Crates**

## Exercice d'exemple 1

Tiré de ["lewisclement"](https://exercism.org/my/solutions/c87d24f45c284c83b0da780d132f1c4b)
...
Tiré de ["yawpitch"](https://exercism.org/tracks/rust/exercises/decimal/solutions/16d3747cccca4ee9aeba5a0e8f0ed429)
