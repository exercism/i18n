...
En JavaScript, les instances d'un [`Array`][arrays-concept] sont des objets ordinaires, enrichis de nombreuses [méthodes utiles][array-docs].
Il inclut la propriété `length`, ainsi que des méthodes pour parcourir, analyser et modifier le tableau.
Les éléments du tableau sont des propriétés indexées de ce tableau.
Au lieu de parcourir manuellement ces propriétés indexées, utilise l'analyse de tableau.
Beaucoup de fonctions natives qui analysent le contenu d'un tableau prennent en argument une fonction qui renvoie vrai ou faux. Une telle fonction s'appelle un [`predicate`][predicate_in_programming]

En général, utiliser les abstractions et les méthodes natives est plus idiomatique, plus lisible et plus facile à maintenir que d'utiliser un équivalent avec `for` ou `.forEach`.
