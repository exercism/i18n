...
Le [pattern matching][pattern-matching] permet d'écrire du code à embranchements expressif, et la [destructuration][destructuring] offre une liaison élégante des valeurs aux variables.

## Le pattern matching simple

Le pattern matching permet de lier des valeurs à des variables, ou de les ignorer avec le joker `_`, en utilisant l'instruction `case`.
...
La destructuration permet de lier des valeurs dans des liaisons `let`, dans les arguments d'une fonction, et bien sûr dans les expressions `case`, dès lors qu'une seule forme est possible pour la donnée.
...
La destructuration s'utilise aussi avec les [enregistrements][records-pattern-matching], ou avec le [mot-clé `as`][as-keyword].
