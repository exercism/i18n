...
Comme une clé peut être présente ou non dans le dictionnaire, le résultat sera enveloppé dans un `Maybe`.
...
Si la clé n'existe pas, la fonction de mise à jour sera appelée avec `Nothing`.
...
Une liste de paires clé-valeur peut être convertie en dictionnaire avec `fromList`.
...
La liste est ordonnée par les clés.
