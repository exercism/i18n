...
L'extrait de code ci-dessous présente les termes les plus courants, à titre de référence.
Comme le C++ ignore les espaces, la mise en forme a été modifiée pour placer chaque élément sur une seule ligne.
...
```
~~~~exercism/advanced
La déclaration fonctionne comme une note adressée au compilateur, indiquant qu'il existe une fonction portant ce nom, ce type de retour et cette liste de paramètres.
...
Les déclarations peuvent résoudre des problèmes comme les références cycliques et servir à séparer l'interface de l'implémentation.
~~~~

## Le qualificatif const

Parfois, tu veux t'assurer que des valeurs ne peuvent pas être modifiées après avoir été initialisées.
...
~~~~exercism/note
Tu verras souvent les constantes écrites en _UPPER_SNAKE_CASE_.
...
Tu peux aussi utiliser `const` comme qualificatif pour les paramètres d'une fonction.
...
Tu verras souvent des références `const` pour des objets dont la copie pourrait être coûteuse, comme des _strings_ plus longues.
...
Cela évite une opération de copie depuis l'objet original qui a été passé à la fonction.
...
Toute modification de la signature de type donne lieu à une nouvelle fonction.

L'exemple `play_sound` comporte six surcharges différentes pour couvrir divers scénarios :
...
Certaines fonctions peuvent devenir très longues, et bon nombre de leurs appels utilisent peut-être les mêmes valeurs pour la plupart des paramètres.
...
Comme la déclaration de la fonction est souvent lue avant la définition, c'est le meilleur endroit pour définir les arguments par défaut.
