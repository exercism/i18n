...
L'instruction de contrôle de flux la plus simple en Java est l'instruction _if-then_.
Cette instruction sert à n'exécuter une section de code que si une condition particulière vaut `true`.
...
L'instruction _if-then-else_ offre un chemin d'exécution alternatif lorsque la condition de la clause `if` prend la valeur `false`.
...
Dans l'exemple ci-dessus, conduire la voiture lorsque le carburant est inférieur ou égal à `5` fera avancer la voiture, mais allumera le voyant de carburant.
Lorsque le carburant atteint `0`, la voiture s'arrêtera.
