...
Voici un récapitulatif de ces méthodes, avec quelques ajouts :
...
Si tu n'as besoin que d'une seule des deux valeurs, tu peux utiliser le symbole spécial `_` pour indiquer que l'autre n'est pas utile.
Cela aide à la fois à la clarté pour le développeur et constitue aussi une optimisation des performances.
...
Tu peux aussi énumérer dans des blocs imbriqués et enchaîner les méthodes les unes à la suite des autres.
Par exemple, si on a un tableau de _hash_ d'animaux et qu'on veut extraire ceux dont le nom est court, on pourrait faire quelque chose comme :
