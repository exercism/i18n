# Εισαγωγή

Η Python έχει δύο δομές επανάληψης.
Τους βρόχους `while` για _αόριστη_ (μη μετρημένη) επανάληψη και τους βρόχους `for` για _ορισμένη_ (μετρημένη) επανάληψη.
Οι λέξεις-κλειδιά `break`, `continue` και `else` βοηθούν να προσαρμόσεις τη συμπεριφορά του βρόχου.
Οι `range()` και `enumerate()` βοηθούν στη μέτρηση και στις θέσεις των στοιχείων μέσα σε έναν βρόχο.


## While

Οι βρόχοι [`while`][while statement] συνεχίζουν να εκτελούνται όσο η `loop expression` ή το "test" αξιολογείται σε `True` μέσα σε ένα [`boolean context`][truth value testing] και σταματούν όταν αυτό αξιολογείται σε `False`:

```python

# Lists are considered "truthy" in a boolean context if they
# contain one or more values, and "falsy" if they are empty.

>>> placeholders = ["spam", "ham", "eggs", "green_spam", "green_ham", "green_eggs"]

>>> while placeholders:
...     print(placeholders.pop(0))
...
'spam'
'ham'
'eggs'
'green_spam'
'green_ham'
'green_eggs'
```


## For

Ο βασικός βρόχος [`for`][for statement] στην Python περιγράφεται καλύτερα ως ένα _`for each`_ που διατρέχει τις τιμές οποιουδήποτε [αντικειμένου iterable][iterable] και σταματά όταν δεν επιστρέφονται άλλες τιμές από την κλήση της [`next()`][next built-in]:

```python

>>> word_list = ["bird", "chicken", "barrel", "bongo"]

>>> for word in word_list:
...    if word.startswith("b"):
...        print(f"{word.title()} starts with a B.")
...    else:
...        print(f"{word.title()} doesn't start with a B.")
...
'Bird starts with a B.'
'Chicken doesn\'t start with a B.'
'Barrel starts with a B.'
'Bongo starts with a B.'
```


## Αντικείμενο ακολουθίας range()

Όταν δεν δίνεται κάποιο συγκεκριμένο `iterable`, η ειδική ακολουθία [`range()`][range] χρησιμοποιείται ως μετρητής βρόχου.
Η `range()` απαιτεί ένα `int` όπου θα `stop` η ακολουθία και μπορεί προαιρετικά να πάρει παραμέτρους `start` και `step`.
Αν δεν δοθεί αριθμός `start`, η ακολουθία ξεκινά από το 0.
Τα αντικείμενα `range()` είναι **τεμπέλικα** (_οι τιμές παράγονται κατ' απαίτηση_), υποστηρίζουν όλες τις [κοινές πράξεις ακολουθίας][common sequence operations] και καταλαμβάνουν σταθερή ποσότητα μνήμης, ανεξάρτητα από το μήκος της ακολουθίας που ορίζεται.

```python
# Here we use range to produce some numbers, rather than creating a list of them in memory.
# The values will start with 1 and stop *before* 7

>>> for number in range(1, 7):
...    if number % 2 == 0:
...       print(f"{number} is even.")
...    else:
...       print(f"{number} is odd.")
'1 is odd.'
'2 is even.'
'3 is odd.'
'4 is even.'
'5 is odd.'
'6 is even.'

# range() can also take a *step* parameter.
# Here we use range to produce only the "odd" numbers, starting with 3 and stopping *before* 15.

>>> for number in range(3, 15, 2):
...    if number % 2 == 0:
...       print(f"{number} is even.")
...    else:
...       print(f"{number} is odd.")
...
'3 is odd.'
'5 is odd.'
'7 is odd.'
'9 is odd.'
'11 is odd.'
'13 is odd.'
```


## Τιμές και θέσεις με τη enumerate()

Αν χρειάζεσαι και τις τιμές και τις θέσεις τους, η ενσωματωμένη [`enumerate(<iterable>)`][enumerate] επιστρέφει ζεύγη (`index`, `value`):

```python

>>> word_list = ["bird", "chicken", "barrel", "apple"]

# *index* and *word* are the loop variables.
# Loop variables can be any valid python name.

>>> for index, word in enumerate(word_list):
...    if word.startswith("b"):
...        print(f"{word.title()} (at index {index}) starts with a B.")
...    else:
...        print(f"{word.title()} (at index {index}) doesn't start with a B.")
...
'Bird (at index 0) starts with a B.'
'Chicken (at index 1) doesn\'t start with a B.'
'Barrel (at index 2) starts with a B.'
'Apple (at index 3) doesn\'t start with a B.'


# The same method can be used as a "lookup" for pairing items between two lists.
# Of course, if the lengths or indexes don't line up, this doesn't work.

>>> word_list = ["cat", "chicken", "barrel", "apple", "spinach"]
>>> category_list = ["mammal", "bird", "thing", "fruit", "vegetable"]

>>> for index, word in enumerate(word_list):
...    print(f"{word.title()} is in category: {category_list[index]}.")
...
'Cat is in category: mammal.'
'Chicken is in category: bird.'
'Barrel is in category: thing.'
'Apple is in category: fruit.'
'Spinach is in category: vegetable.'
```


## Αλλαγή της συμπεριφοράς του βρόχου

Η λέξη-κλειδί [`continue`][continue statement] μπορεί να χρησιμοποιηθεί για να παρακάμψεις και να προχωρήσεις στον επόμενο κύκλο επανάληψης:

```python
word_list = ["bird", "chicken", "barrel", "bongo", "sliver", "apple", "bear"]

# This will skip *bird*, at index 0
for index, word in enumerate(word_list):
    if index == 0:
        continue
    if word.startswith("b"):
        print(f"{word.title()} (at index {index}) starts with a b.")

'Barrel (at index 2) starts with a b.'
'Bongo (at index 3) starts with a b.'
'Bear (at index 6) starts with a b.'
```


Η λέξη-κλειδί [`break`][break statement] (_όπως σε πολλές γλώσσες συγγενικές με τη C_) μπορεί να χρησιμοποιηθεί για να σταματήσει η επανάληψη και να "σπάσεις" τον πιο εσωτερικό `loop` που την περιβάλλει:

```python
>>>  word_list = ["bird", "chicken", "barrel", "bongo", "sliver", "apple"]

>>> for index, word in enumerate(word_list):
...    if word.startswith("b"):
...        print(f"{word.title()} (at index {index}) starts with a B.")
...    elif word == "sliver":
...       break
...    else:
...       print(f"{word.title()} doesn't start with a B.")
... print("loop broken.")
...
'Bird (at index 0) starts with a B.'
'Chicken doesn\'t start with a B.'
'Barrel (at index 2) starts with a B.'
'Bongo (at index 3) starts with a B.'
'loop broken.'
```

[break statement]: https://docs.python.org/3/reference/simple_stmts.html#the-break-statement
[common sequence operations]: https://docs.python.org/3/library/stdtypes.html#common-sequence-operations
[continue statement]: https://docs.python.org/3/reference/simple_stmts.html#the-continue-statement
[enumerate]: https://docs.python.org/3/library/functions.html#enumerate
[for statement]: https://docs.python.org/3/reference/compound_stmts.html#for
[iterable]: https://docs.python.org/3/glossary.html#term-iterable
[next built-in]: https://docs.python.org/3/library/functions.html#next
[range]: https://docs.python.org/3/library/stdtypes.html#range
[truth value testing]: https://docs.python.org/3/library/stdtypes.html#truth-value-testing
[while statement]: https://docs.python.org/3/reference/compound_stmts.html#the-while-statement
