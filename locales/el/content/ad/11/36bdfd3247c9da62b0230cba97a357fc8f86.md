...
Τα λεξικά χειρίζονται με συναρτήσεις και τελεστές που ορίζονται στο [module `Dict`][dict-module].

Οι σχολιασμοί τύπων για τα λεξικά γράφονται ως εξής
...
Τα λεξικά μπορούν να δημιουργηθούν ως εξής:
...
Τα στοιχεία μπορούν να ανακτηθούν με τη `get`.
Καθώς ένα κλειδί μπορεί να υπάρχει ή να μην υπάρχει στο λεξικό, το αποτέλεσμα θα είναι τυλιγμένο σε ένα `Maybe`.
...
Τα στοιχεία μπορούν να προστεθούν με τη `insert`.
Τα στοιχεία για κλειδιά που υπάρχουν ήδη στο λεξικό αντικαθίστανται.
...
Τα στοιχεία μπορούν να ενημερωθούν με τη `update`.
...
Τα στοιχεία μπορούν να αφαιρεθούν με τη `remove`.
...
Μια λίστα από ζεύγη κλειδιού-τιμής μπορεί να μετατραπεί σε λεξικό με τη `fromList`.
...
Τα στοιχεία μπορούν να μετατραπούν σε λίστα με τη `toList`.
...
Πολλαπλά στοιχεία μπορούν να αφαιρεθούν με τη `filter`.
...
Τα στοιχεία μπορούν να μετασχηματιστούν με τη `map`.
...
Τα λεξικά μπορούν να συνδυαστούν / μετασχηματιστούν με τη `merge`.
...
Οποιαδήποτε συνάρτηση ή τελεστής φαίνεται να τροποποιεί ένα λεξικό (όπως η προσθήκη ενός στοιχείου), στην πραγματικότητα επιστρέφει ένα νέο λεξικό.
Η απόδοση συνήθως δεν αποτελεί πρόβλημα, καθώς η υλοποίηση αποτρέπει περιττές δεσμεύσεις χώρου και αντιγραφές.

Στην Elm, είναι γενικά καλύτερα να χρησιμοποιείς αφαιρέσεις υψηλότερου επιπέδου, όπως τις `Dict.map`, `Dict.filter` και `Dict.merge`, αντί για αφαιρέσεις χαμηλότερου επιπέδου, όπως τις `Dict.get` και `Dict.remove`, αν και φυσικά αυτό εξαρτάται από το πλαίσιο.
