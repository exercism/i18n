...
Τα λεξικά χειρίζονται με συναρτήσεις και τελεστές που ορίζονται στη [μονάδα `Dict`][dict-module].

Οι σημειώσεις τύπου για τα λεξικά γράφονται ως εξής
...
Μπορείς να ανακτήσεις στοιχεία με τη `get`.
Επειδή ένα κλειδί μπορεί να υπάρχει ή να μην υπάρχει στο λεξικό, το αποτέλεσμα θα είναι τυλιγμένο σε ένα `Maybe`.
...
Μπορείς να προσθέσεις στοιχεία με τη `insert`.
...
Μπορείς να ενημερώσεις στοιχεία με τη `update`.
...
Μπορείς να αφαιρέσεις στοιχεία με τη `remove`.
Αν το κλειδί δεν υπάρχει, δε γίνεται καμία αλλαγή.
...
Μπορείς να μετατρέψεις στοιχεία σε λίστα με τη `toList`.
Η λίστα είναι ταξινομημένη με βάση τα κλειδιά.
...
Μπορείς να αφαιρέσεις πολλά στοιχεία με τη `filter`.
...
Μπορείς να μετασχηματίσεις στοιχεία με τη `map`.
...
Οποιαδήποτε συνάρτηση/τελεστής φαίνεται να τροποποιεί ένα λεξικό (για παράδειγμα προσθέτοντας ένα στοιχείο), στην πραγματικότητα επιστρέφει ένα νέο λεξικό.
Η απόδοση συνήθως δεν είναι πρόβλημα, καθώς η υλοποίηση αποτρέπει περιττές δεσμεύσεις/αντιγραφές μνήμης.

Στην Elm, γενικά είναι καλύτερα να χρησιμοποιείς αφαιρέσεις υψηλότερου επιπέδου, όπως τις `Dict.map`, `Dict.filter` και `Dict.merge`, αντί για αφαιρέσεις χαμηλότερου επιπέδου, όπως τις `Dict.get` και `Dict.remove`, αν και φυσικά αυτό εξαρτάται από τα συμφραζόμενα.
