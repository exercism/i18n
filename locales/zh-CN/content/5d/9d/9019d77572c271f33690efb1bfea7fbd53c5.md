...
在 JavaScript 中，[`Array`][arrays-concept] 的实例是普通对象，它们带有很多[实用的方法][array-docs]。
其中包括`length`属性，以及用于遍历、分析和修改数组的方法。
数组的元素就是该数组的索引属性。
与其手动遍历这些索引属性，不如使用数组分析。
许多用于分析数组内容的内置函数，都会接收一个返回 true 或 false 的函数作为实参。这样的函数叫做 [`predicate`][predicate_in_programming]。

总的来说，使用内置方法这类抽象，比使用`for`或`.forEach`的等价写法更符合习惯、更易读、也更好维护。
