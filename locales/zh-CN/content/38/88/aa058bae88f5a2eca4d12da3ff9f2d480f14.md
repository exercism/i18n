...
Swift 提供了一种内置类型 [`Bool`][bools]，用来表示真值：`true`和`false`。

## 逻辑运算符

Swift 提供了三个[逻辑运算符][logical-operators]（`!`、`&&`和`||`），用于组合并对布尔表达式求值。

### 逻辑与（`&&`）

[_与_运算符][and]（`&&`）只有在两个操作数都为`true`时才返回`true`。
只要有一个操作数为`false`，它就返回`false`。
在`&&`符号的两侧各放一个表达式或布尔值：
...
[_或_运算符][or]（`||`）只要**至少一个**操作数为`true`就返回`true`。
...
[_非_运算符][not]（`!`）用于对一个布尔值取反。
如果值为`false`，它返回`true`；如果值为`true`，它返回`false`。
把`!`运算符直接放在值或表达式的前面：
...
你可以用[括号][explicit-parentheses]对表达式分组，并控制求值顺序。
...
~~~~exercism/note
需要澄清求值顺序，或者想让复杂条件更易读时，就用括号。
如果默认优先级已经很清楚，就不要添加多余的括号。
~~~~
