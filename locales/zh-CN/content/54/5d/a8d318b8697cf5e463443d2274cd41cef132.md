...
**浮点数**是一种可以带有小数部分的数字，例如`-1.23`或`3.1415`。

为了把这样的数字表示成一串字节，x86-64 使用**IEEE-754**标准。该标准中最常见的两种格式是单精度浮点数和双精度浮点数。x86-64 两种都支持。

单精度浮点数宽 32 位，而双精度浮点数宽 64 位。

## XMM 寄存器

在 x86-64 中，有 16 个专门用于浮点运算的寄存器，命名为`xmm0`到`xmm15`。

这些寄存器至少宽 128 位，但较新的处理器将其提升到 256 位或 512 位。不过实际上，大多数浮点数只使用最低的 32 位或 64 位，具体取决于所需的精度。
...
请注意，可以把 32 位整数转换为 64 位浮点数，也可以把 64 位整数转换为 32 位浮点数。同样，也可以把 32 位浮点数转换为 64 位整数，或把 64 位浮点数转换为 32 位整数。换句话说，`cvtxx2yy`中两个操作数的大小_不必相同_：
...
当浮点数无法精确表示为整数时，`cvtxx2si`会根据一个名为`mxcsr`的特殊寄存器中的舍入控制来舍入该数字。它可以是就近舍入、向下舍入、向上舍入或截断。函数入口处没有保证的默认值。

请注意，精确的结果不会被舍入，因此也不受舍入控制的影响。例如，`42.0`_总是_会被转换为`42`。

还有一系列格式为`cvttxx2si`（多了一个`t`）的指令，它们总是截断结果。

## 舍入

`roundxx`系列指令把源操作数中的浮点数转换为舍入后的浮点数，并存放到目标操作数中。

这里，`xx`照例是操作数类型的占位符：`ss`表示单精度浮点数，`sd`表示双精度浮点数。

这些指令把常量数字作为第三个操作数。这个数字表示舍入模式：
...
由于这些指令会产生精确的结果，因此可以用它们来为`cvtxx2si`做准备。

## 算术指令

许多浮点数运算所用的指令，名称与整数的对应指令相似，只是在末尾加上必要的占位符（`ss`或`sd`）。这些指令都是双操作数，语义和往常一样，即使执行乘法或除法的指令也是如此：
...
把一个 XMM 寄存器移动到另一个寄存器的指令是`movss`或`movsd`：
...
**无法**直接把立即数（一个常量有符号整数）移动到 xmm 寄存器中。需要先把该数字移动到通用寄存器，再用`cvtsi2xx`进行转换：
...
有两组指令用于对两个浮点数进行比较。它们的格式是`comixx`和`ucomixx`，其中`xx`要么是`ss`，要么是`sd`。两者都像往常一样设置`rflags`中的标志位，语义与`cmp`相同。

它们之间的区别在于：如果两个浮点数中有一个的值是`NaN`，`ucomixx`会设置`parity flag (PF)`，而`comixx`在这种情况下会抛出异常。
...
在 System V ABI 中，前八个浮点实参按顺序通过`xmm0`到`xmm7`传给函数。浮点值通过`xmm0`返回，必要时还会通过`xmm1`返回。

所有`xmm`寄存器都可以自由使用，没有任何一个是为调用方保留的。

## 内存

浮点值可以按照与同大小整数相同的语义在内存中定义。NASM（The Netwide Assembler，本轨道使用的汇编器）还允许用科学计数法声明浮点值。
