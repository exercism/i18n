...
Go 中的[`Time`][time]是一种用于描述时间点的类型。
日期和时间信息可以通过它的各个方法进行访问、比较和操作，不过也有一些函数是直接对`time`包调用的。
当前的日期和时间可以通过[`time.Now`][now]函数获取。

[`time.Parse`][parse]函数会把字符串解析成`Time`类型的值。
Go 定义解析所需布局的方式很特别。
你需要用下面这个特殊时间戳里的值写出一个布局示例：
...
和`Parse`函数一样，目标布局同样通过一个示例来定义，这个示例使用的是特殊时间戳里的值。
...
要自定义布局，可以把这些选项组合起来使用。
Go 也提供了一些预定义的日期和时间戳[格式常量][const]。
...
| 日        | 02 ; 2 ; \_2（用于前导 0）                 |
| 星期     | Mon ; Monday                                   |
| 小时        | 15（24 小时制）; 3 ; 03（AM 或 PM） |
| 分        | 04 ; 4                                         |
| 秒        | 05 ; 5                                         |
| 上午/下午标记  | PM                                             |
| 一年中的第几天 | 002 ; \_\_2                                    |

`time.Time`类型有多种方法可以访问某个特定的时间。例如，小时：[`Time.Hour()`][hour]，月份：[`Time.Month()`][month]。
关于它的工作方式，可以在[官方文档][time]中了解更多。

[`time`][time]包还包含另一种类型[`Duration`][duration]，它表示流逝的时间；此外还支持位置/时区、定时器以及其他相关功能，这些会在另一个概念里介绍。
