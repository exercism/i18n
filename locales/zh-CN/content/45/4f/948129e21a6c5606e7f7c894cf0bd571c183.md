...
JavaScript 内置了一个 `Date` 对象，用于存储日期和时间，并提供管理它们的方法。

<!-- prettier-ignore -->
~~~exercism/caution
它基于 Java 的 `java.util.Date` 类，而那个类在 2010 年代初就被取代了，但为了向下兼容，JavaScript 的 `Date` 一直保留至今。

由于 Date 总体上很难处理，而且时区的支持要么很差、要么根本不存在，于是出现了许多库，比如 `moment.js`、`day.js`、`date-fns` 和 `luxon`。
这些库在 Exercism 上都不可用。

在你自己的项目中，不要使用像 `moment.js` 这样已弃用或无人维护的包，而应依赖更现代的替代方案，比如 `luxon`，或者尚未广泛可用的 [Temporal][mdn-temporal]。
本练习聚焦于 `Date`，它在 JavaScript 走到尽头之前都会一直重要。
...
人们可能以为 `new Date(0)` 会生成“最早”的日期对象，但 JavaScript 会把日期转换到你的本地时区，这意味着只有身处 [GMT / UTC+0][defn-gmt] 时区附近的人才会真正得到 [Unix 纪元][defn-unix-epoch]值。

### ISO 8601 时间戳（字符串）

你可以把一个表示日期的字符串值传给 `Date` 构造函数。
在各实现之间保持一致的**唯一**格式，是国际公认且已标准化的所谓 [ISO 8601 时间戳字符串][defn-iso8601]的[简化版本][mdn-date-string-format]。

在 [UTC][defn-gmt] 下的某个时间点看起来像这样：
...
> 解释为 UTC 时间源于一个历史上的规范错误，它与 ISO 8601 不一致，但出于 Web 兼容性无法更改。
> 参见 [坏掉的解析器：一个 Web 现实问题][ref-broken-parser]。

### Date 对象

也可以把一个已有的日期对象用作构造函数的实参。
这会复制已有的 `Date` 对象，日期和时间都相同。
...
传入三个数字即可创建一个表示日期的日期对象。
传入 6 个数字则可以创建一个表示日期和时间的日期对象。
...
第二个值是 `month`，从一月对应的 `0` 开始，一直到十二月对应的 `11`。

## `Date.parse()`

你可能会看到有关日期解析函数 `Date.parse` 的提及或引用。
由于这个函数只有少数几个不变式（也就是必然成立的事实），而其余的实现并未规定（因而也不标准），所以不应该使用它。

## 访问 `Date` 的各个组成部分

日期对象上有许多方法可以返回日期的各个组成部分：
...
其中每个方法都有一个对应的 `set` 变体（例如 `setFullYear`）来更新值。
任何溢出的值都会进位到下一个组成部分：
...
日期对象有一个 `getTime()` 方法，返回以毫秒为单位的 UNIX 时间戳，即自 1970 年 1 月 1 日 UTC 零时起经过的毫秒数。
此外，还有一个 `toISOString()` 方法，可以把日期对象转换为 ISO 8601 时间戳字符串。

## 比较日期

大于（`>`）、大于等于（`>=`）、小于（`<`）和小于等于（`<=`）可以直接用于两个日期之间，或日期与数字之间。
这样可行，是因为 JavaScript 会尝试把日期强制转换为原始值。

<!-- prettier-ignore -->
~~~exercism/advanced
当比较两个日期，或在日期与数字之间进行比较时，JavaScript 会调用 [`[Symbol.toPrimitive]("number")`][mdn-to-primitive]，而它在内部又会调用 [`date.valueOf()`][mdn-date-value-of]。
后者等同于调用 [`date.getTime()`][mdn-date-get-time]。

如果你不想依赖这种行为，请先用 `getTime()` 转换为数字。
