...
所接受的正则表达式[语法][regexp-syntax]与 Perl、Python 等语言所用的通用语法相同。
...
函数`MustCompile`是`Compile`的便捷替代方案：
...
`Regexp`有 16 个方法可以匹配正则表达式并定位匹配到的文本。
...
* 如果含有`All`，该方法会依次匹配整个表达式所有互不重叠的匹配项。
* 如果含有`String`，实参是字符串；否则是字节切片；返回值也会相应地调整。
* 如果含有`Submatch`，返回值是一个切片，标识表达式中依次匹配到的各个子匹配。
* 如果含有`Index`，匹配项和子匹配项由输入字符串中的字节下标对来标识。
...
下面我们会演示几个方法的用法。
关于这些函数以及其他函数的详细信息，请参阅 [API 文档][package-regexp]。

### `MatchString` 示例

方法`MatchString`用于报告字符串中是否包含正则表达式的匹配项。
...
方法`FindString`返回一个字符串，其中保存着正则表达式最左侧匹配项的文字。
...
方法`FindStringSubmatch`返回一个字符串切片，其中保存着正则表达式最左侧匹配项的文字，以及它的子表达式（如果有）匹配到的文字。
这可以用来找出与捕获组匹配的字符串。
返回值为`nil`表示没有匹配。
...
方法`re.ReplaceAllString(src,repl)`返回`src`的副本，并把正则表达式`re`的匹配项替换为替换字符串`repl`。
...
方法`re.Split(s,n)`会把文本`s`按该表达式切分成子字符串，并返回这些表达式匹配项之间的子字符串所组成的切片。
数量`n`决定最多返回多少个子字符串。
如果`n<0`，该方法返回所有子字符串。
