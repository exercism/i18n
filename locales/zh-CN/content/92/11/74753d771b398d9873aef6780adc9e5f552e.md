...
在这个实例上调用`add`、`remove`这类方法，会在运行时抛出异常。

要创建可修改的`Set`，需要实例化一个实现了`Set`接口的类。
实现该接口最常用的内置类是[`HashSet`][hashset-docs]类。
...
不过，与`Collection`接口的一个显著区别是，`add`、`remove`这类方法返回的是`boolean`（而不是`void`），用来表示调用该方法时集合中是否包含该项：
