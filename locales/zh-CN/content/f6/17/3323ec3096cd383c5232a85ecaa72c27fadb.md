...
在 C 语言中，[struct][struct] 是一种复合类型，由不同的字段组成。每个字段都有自己的类型和大小。

例如，下面定义的类型由三个整数组成，其中两个占 4 字节，一个占 8 字节：
...
如果 struct 的某个字段是数组，那么为这个数组分配的所有字节都属于这个 struct。但如果这个字段只是指向数组的指针，那么数组就必须存储在别的地方：
...
一个类型的[对齐][alignment] 是一个非负的 2 的幂（1、2、4、8……），它表示对该类型地址的一项要求。这种类型的每个对象都必须位于其对齐值的整数倍的地址上。

对于基本类型，对齐值通常等于它的大小，因此，例如`int32_t`通常按 4 字节对齐。这意味着`int32_t`必须分配在 4 字节的边界上。

而 struct 的对齐值，则是它所有字段中最大的那个对齐值。所以，例如上面定义的`struct example`，其对齐值就是 8 字节，因为这就是`uint64_t`的对齐值。

如果某个 struct 字段的对齐值小于整个 struct 的对齐值，系统可能会在该字段之后插入填充字节。这能确保后续字段以及整个 struct 都满足对齐要求。实际上，这意味着尽管这些填充字节会计入 struct 的大小，但它们并不构成 struct 的值：
...
注意，在上面定义的 struct 中，字段 'one_byte' 的对齐值为 1，可以放在任意地址上。由于字段 'four_byte' 的对齐值为 4，struct 中的字段也必须遵守 4 字节的边界。这意味着 struct 中任意两个字段之间，相隔的字节数都必须是 4 的倍数。由于 'one_byte' 只占 1 字节，所以要在它之后插入 3 个填充字节，以满足这一边界要求。

相邻的字段会尽可能被归到一组，以尽量减少所需的填充字节数：
...
这意味着，改变 struct 中字段的顺序，有时会导致其大小发生变化：
...
在汇编中访问 struct 时，务必把可能存在的填充字节考虑在内。

## 调用约定

一般来说，值传入函数和从函数返回的方式，都与人们的预期一致：

- 整数值，包括内存地址和枚举，通过通用寄存器（GPR）传递。
- 浮点值通过`xmm`寄存器传递。

不过，struct 是个特殊情况。按经验法则，struct 中的所有字段在内存中占据相邻的空间，应尽可能放入同一个寄存器：
...
如果一个 struct 既有整数值字段，又有浮点值字段，那么 struct 中每个连续的 8 字节都应单独分类：
...
如果整数和浮点值位于同一个 8 字节中，这个类型就按整数处理，整个 8 字节（包括其中的浮点值）都会被放入 GPR：
...
~~~~exercism/caution
用`cvtss2si`或`cvtsd2si`把浮点数转换为整数时，会改变底层字节的组织方式。

你可以使用 [movd][mov]（用于 32 位值）或 [movq][mov]（用于 64 位值），把原始字节从`xmm`寄存器移动到 GPR，而不改变位表示形式。

[mov]: https://www.felixcloutier.com/x86/movd:movq
~~~~

不过，大小超过 16 字节的 struct 通常会放在内存中，不论每个 8 字节是什么类型：
...
当 struct 作为实参传给函数时，它会按需要用掉尽可能多的寄存器。如果它被放在内存中，则改用栈。

从函数返回 struct 时也适用类似的规则。它们通常首先通过`rax`和`rdx`返回（如果是整数值），或通过`xmm0`和`xmm1`返回（如果是浮点值）。但如果它被放在内存中，调用方就必须为它预留空间，并把这块内存位置的地址作为函数的隐式第一个实参（通过`rdi`）传入。函数随后把 struct 存入这块空间，并在`rax`中返回它的地址。
