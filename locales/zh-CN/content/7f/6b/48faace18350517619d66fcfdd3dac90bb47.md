...
`MapSet`是唯一值的集合，在 Elixir 中用来表示集合。它可以包含任意类型的值，而且没有顺序的概念。

注意，`Set`模块也存在，但已弃用，推荐改用`MapSet`。

你可以使用`MapSet.new/0`、`MapSet.new/1`和`MapSet.new/2`创建集合，并使用`MapSet.to_list/1`把它们转换成数组。
...
注意，由于`MapSet`没有顺序的概念，`MapSet.to_list/1`不保证返回有序的数组。

你可以使用`MapSet.put/2`和`MapSet.delete/2`添加或删除元素。

你可以使用`MapSet.size/1`和`MapSet.member?/2`查询集合的内容，并使用`MapSet.equal?/2`、`MapSet.subset?/2`和`MapSet.disjoint?/2`比较集合。
...
多个 MapSet 可以通过`MapSet.union/2`、`MapSet.intersection/2`、`MapSet.difference/2`和`MapSet.symmetric_difference/2`组合起来。
...
你可以使用`MapSet.filter/2`、`MapSet.reject/2`和`MapSet.split_with/2`过滤和拆分集合。
