...
- [/r/WebAssembly](https://www.reddit.com/r/WebAssembly/) 是 WebAssembly 的子版块。
- [Github issue 跟踪器](https://github.com/exercism/wasm/issues) 是我们跟踪 Exercism 中 JavaScript 练习的开发和维护情况的地方。如果以上链接都帮不了你，欢迎在这里发 issue。

## 如何调试

与许多语言不同，WebAssembly 代码无法自动访问控制台之类的全局资源。这类功能必须通过导入来提供。

为了提供类似`console.log`的功能以及一些其他便利，Exercism WebAssembly track 为所有练习提供了一套标准函数库。

这些函数必须在你的 WebAssembly 模块顶部导入，之后就能在 WebAssembly 代码中调用。

`log_mem_*`系列函数需要能够访问你的 WebAssembly 模块的线性内存。默认情况下，这是私有状态，因此要让它可访问，你必须以导出名`mem`导出线性内存。做法如下：
...
我们为 WebAssembly 的每种原始类型都提供了日志函数。这在记录全局变量和局部变量时很有用。

### log_i32_s - 将 32 位有符号整数输出到控制台
...
我们提供的日志函数可以将线性内存中某个地址范围内的数据解释为特定类型的静态数组。这类似于 JavaScript 的 TypedArrays。长度参数不是以字节为单位，而是以与该函数相关联的类型的连续元素个数为单位。

**要让这些函数正常工作，你的 WebAssembly 模块必须使用命名导出“mem”来声明并导出它的线性内存**

```wasm
(memory (export "mem") 1)
```

### log_mem_as_utf8 - 将一串 UTF8 字符输出到控制台
...
### log_mem_as_i8 - 将一串 8 位有符号整数输出到控制台
...
### log_mem_as_u8 - 将一串 8 位无符号整数输出到控制台
...
### log_mem_as_i16 - 将一串 16 位有符号整数输出到控制台
...
### log_mem_as_u16 - 将一串 16 位无符号整数输出到控制台
...
### log_mem_as_i32 - 将一串 32 位有符号整数输出到控制台
...
### log_mem_as_u32 - 将一串 32 位无符号整数输出到控制台
...
### log_mem_as_i64 - 将一串 64 位有符号整数输出到控制台
...
### log_mem_as_u64 - 将一串 64 位无符号整数输出到控制台
...
### log_mem_as_u64 - 将一串 32 位浮点数输出到控制台
...
### log_mem_as_f64 - 将一串 64 位浮点数输出到控制台
