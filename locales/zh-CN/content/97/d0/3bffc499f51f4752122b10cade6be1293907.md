...
字典存储的是唯一的键与对应值之间的无序关联。
字典中的所有键都必须是同一类型，所有的值也必须共用同一类型。

字典字面量写成用逗号分隔的 `key: value` 对，并放在方括号里：
...
字典类型有两种显式写法：`Dictionary<Key, Value>`，或者简写语法 `[Key: Value]`。
...
可以使用下标来访问值：把键放在字典名后面的方括号里。
因为某个键可能不存在于字典中，所以下标访问返回一个*可选值*（`Value?`）。
如果键存在，Swift 返回包装其中的值；如果不存在，则返回 `nil`：
...
要避免处理可选值，可以用`default`形参提供一个默认值：
...
对于用`var`声明的可变字典，可以使用下标赋值来添加或更新键值对：
...
因为字典是无序的，对它排序会得到一个由键值对组成的数组。
可以给`sorted(by:)`方法传入一个闭包，它比较两个`(key, value)`元组，如果第一个元组应当排在第二个之前，就返回`true`：
