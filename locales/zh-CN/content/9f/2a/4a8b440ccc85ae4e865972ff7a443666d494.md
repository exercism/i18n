...
为了把这样的数字表示成一串字节，x86-64 使用**IEEE-754**标准。
该标准中最常见的两种格式是单精度浮点数和双精度浮点数。
x86-64 两种都支持。
...
在 x86-64 中，有 16 个专用于浮点运算的寄存器，名为`xmm0`到`xmm15`。

这些寄存器至少有 128 位宽，但较新的处理器把这一宽度提高到 256 位或 512 位。
不过在实践中，大多数浮点数只用到最低的 32 位或 64 位，具体取决于所需的精度。
...
注意，可以把 32 位整数转换成 64 位浮点数，也可以把 64 位整数转换成 32 位浮点数。
同样，也可以把 32 位浮点数转换成 64 位整数，或把 64 位浮点数转换成 32 位整数。
换句话说，`cvtxx2yy`的两个操作数的大小_不必相同_：
...
当浮点数无法被精确表示成整数时，`cvtxx2si`会根据一个名为`mxcsr`的特殊寄存器中的舍入控制对该数字进行舍入。
这可能是就近舍入、向下舍入、向上舍入或截断。
函数入口处没有保证的默认值。

注意，精确的结果不会被舍入，因此也不受舍入控制的影响。
例如，`42.0`_总是_会被转换成`42`。

还有一系列格式为`cvttxx2si`的类似指令（多了一个`t`），它们总是截断结果。

### 舍入

`roundxx`系列指令把源操作数中的浮点数转换成舍入后的浮点数，存放到目标操作数中。
...
许多浮点数运算所用的指令，其名字与对应的整数指令相似，只是在末尾加上必要的占位符（`ss`或`sd`）。
这些指令都是双操作数的，语义与通常一样，即使是执行乘法或除法的指令也是如此：
...
把一个 XMM 寄存器中的值移动到另一个寄存器，用的是`movss`或`movsd`指令：
...
**不**可以把立即数（一个常量有符号整数）直接移动到 xmm 寄存器。
这个数字需要先移动到通用寄存器，再用`cvtsi2xx`转换：
...
有两组指令用于比较两个浮点数。
它们的格式是`comixx`和`ucomixx`，其中`xx`是`ss`或`sd`。
两者都像通常那样设置`rflags`中的标志位，语义与`cmp`相同。

两者的区别在于：如果两个浮点数中有一个的值是`NaN`，`ucomixx`会设置`parity flag (PF)`，而`comixx`在这种情况下会抛出异常。
...
在 System V ABI 中，前八个浮点实参按顺序通过`xmm0`到`xmm7`传给函数。
浮点值通过`xmm0`返回，必要时也通过`xmm1`。

所有`xmm`寄存器都可自由使用，没有一个是保留给调用方的。

### 内存

浮点值可以按照与同尺寸整数相同的语义在内存中定义。
NASM（The Netwide Assembler，本 track 使用的汇编器）也允许用科学计数法声明浮点值。
