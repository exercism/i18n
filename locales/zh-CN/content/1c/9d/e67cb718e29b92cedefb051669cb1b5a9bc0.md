...
[模式匹配][pattern-matching]能写出富有表现力的分支代码，[解构][destructuring]则能把值优雅地绑定到变量上。

## 简单的模式匹配

使用`case`语句时，模式匹配可以把值绑定到变量，也可以用通配符`_`把值丢弃。
...
只要数据只有一种可能的形态，解构就能在`let`绑定、函数形参，当然还有`case`表达式里绑定值。
