...
如需了解这些主题的更多详细信息，请访问[概念页面][exercism-concept]。

## 通用语法

PHP 起始标签`<?php`标志着 PHP 代码的开始。
所有语句都必须以`;`结尾，用于分隔指令。
...
为了便于阅读，所有代码示例都省略了 PHP 起始标签`<?php`。
...
使用赋值运算符`=`，可以把值赋给一个变量。
变量名必须以美元符号`$`开头，并遵循[命名规范][exercism-concept-naming-conventions]。
