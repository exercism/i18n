...
汇编中的任何数据都是字节序列。这意味着任何数据都可以看作由这些底层字节组成的**数组**。

### 访问数组中的元素

要访问内存中的任何值，都必须先计算它的**有效地址**。有效地址是一个表达式，可以包含以下部分：

- 一个_基址_，例如变量标签所指的地址。
- 一个_索引寄存器_，可以按 `1`、`2`、`4` 或 `8` 进行缩放。
- 一个有符号 32 位_位移量_，即要与基址相加（或从基址中相减）的数字。

注意这个地址是_从 0 开始索引_的，因此不带任何偏移量的基址指向变量的第一个字节。这就是为什么只用一个标签、不带任何偏移量去解引用内存（正如我们在之前的概念中所做的那样），得到的是从第一个字节开始的值：
...
~~~~exercism/note
x86-64 采用[小端序][endianness]。这意味着多字节值的最低有效字节存储在最小的内存地址上。

因此，在访问多字节值的各个字节时，需要给基址加上正的偏移量才能到达更有效的字节。

[endianness]: https://en.wikipedia.org/wiki/Endianness
~~~~

用一组值声明的已初始化变量就是由这些值组成的数组，每个元素都具有指定的大小。不过，偏移量始终以**_字节_**为单位，即使数组中每个元素的大小更大也是如此：
...
注意，由于地址以字节为单位，可以直接访问数组中任意元素的单个字节：
...
~~~~exercism/caution
汇编不会对数组访问做边界检查。超出数组末尾的下标会读取或写入数组之外的内存。这可能导致悄无声息的错误结果、破坏其他数据，而且如果访问的内存未映射，还会导致运行时故障。程序员有责任让每个有效地址都落在数组范围内。
~~~~

### 计算已初始化数组的大小

NASM（Netwide Assembler，本轨道使用的汇编器）有一个特殊符号（`$`），用于指示内存中的当前位置。这个符号提供了一种便捷的方法来计算数组占用的内存总大小：
...
当紧跟在 `example` 之后使用时，`$` 表示数组末尾在内存中的地址。这意味着 `$ - example` 就是该地址与标签 `example` 所指地址之差。这恰好是 `example` 所指数组占用的总大小，以字节为单位。

所以，用这个值定义 `example_length`，我们就创建了一个 8 字节的变量，用来存储数组的大小（即长度），以字节为单位。

### .bss 段

未初始化的数据声明在 **.bss 段**中。在大多数平台上，操作系统会在程序启动时把这些数据填充为零。

在 NASM 中，未初始化的变量有一个名字、一条指示数据大小的伪指令，以及要保留的元素个数。这三者之间以空格分隔。
...
例如，下面这行代码为名为 `example` 的变量保留了 `3` 字节的未初始化空间：
...
而下面这行代码为名为 `arr` 的变量保留了 `10` 乘 `8` 字节（即 `80` 字节）的未初始化空间：
...
`section .bss`中的变量是可变的，也就是说它们可读可写。

未初始化的数据可以像已初始化的数据那样，通过使用有效地址来访问。同样，它们也具有静态存储期，并且可以从同一源文件中的任何函数访问（如果声明为 `global`，还可以从其他源文件访问）。

### 使用 lea 指令访问数组地址

正如之前某个概念中探讨过的，`lea` 指令会计算有效的内存地址。按照相同的语义，可以用它计算数组中任意元素的地址。

注意，在使用相对寻址时，通常需要先把变量的地址加载到寄存器中，然后再构造更复杂的地址：
...
~~~~exercism/note
`lea` 的优点之一是，它使用地址计算算术来求值，而不会真正访问该地址处的内存。这使得即使没有任何寄存器保存内存地址，它也能用于算术运算：
