...
集合是一种不可变、无序的数据结构，其中的值都是唯一的。集合里的所有元素都必须是同一类型。

`gleam/set` 模块定义了 `Set` 类型，并提供了处理集合的各种函数。

你可以用`set.to_list`和`set.from_list`在数组和集合之间相互转换。集合中元素的顺序是不确定的。
...
你可以用`set.new`、`set.insert`和`set.delete`来创建和填充集合。
...
你可以用`set.contains`和`set.size`这两个函数来查询集合的内容。
...
集合可以用`set.union`和`set.intersection`来合并。
