...
[`Set`][set-docs]是一种无序集合，与`List`不同，它保证不包含任何重复值。

`Set`接口的泛型类型参数表示该`Set`中元素的类型：
...
注意，`Set.of()`方法创建的是一个[_不可修改_][unmodifiable-set-docs]的`Set`实例。在这个实例上尝试调用`add`和`remove`之类的方法，会在运行时抛出异常。

要创建可修改的`Set`，需要实例化一个实现`Set`接口的类。实现该接口最常用的内置类是[`HashSet`][hashset-docs]类。

```java
Set<Integer> ints = new HashSet<>();
```

`Set`接口继承自[`Collection`][collection-docs]和[`Iterable`][iterable-docs]接口，因此与其他类型的集合共享许多方法。不过它与`Collection`接口有一个显著区别：`add`和`remove`之类的方法返回的是`boolean`（而不是`void`），用来表示调用该方法时集合中是否包含该项：
