...
在 C 语言中，**struct**是一种复合类型，由不同的字段组成。每个字段都有自己的类型和大小。

例如，下面定义了一个由三个整数组成的类型，其中两个占 4 字节，一个占 8 字节：
...
如果 struct 的某个字段是数组，那么为该数组分配的所有字节都属于这个 struct。但如果该字段只是指向数组的指针，那么数组必须存储在别的地方：
...
类型的**对齐**是一个非负的 2 的幂（1、2、4、8……），表示对其地址的一项要求。该类型的每个对象都必须位于地址为其对齐值倍数的位置上。

对于基本类型，对齐值通常等于它的大小，因此，例如`int32_t`通常有 4 字节的对齐。这意味着`int32_t`必须分配在 4 字节边界上。

不过，struct 的对齐值是它所有字段中对齐值的最大值。例如，上面定义的`struct example`的对齐值是 8 字节，因为这就是`uint64_t`的对齐值。

如果某个 struct 字段的对齐值小于 struct 的对齐值，可能会在该字段之后插入填充字节。这样可以确保后续字段以及整个 struct 都满足对齐要求。实际上，这意味着尽管这些填充字节会算进 struct 的大小，但它们并不构成 struct 的值：
...
注意，在上面定义的 struct 中，字段“one_byte”的对齐值为 1，可以放在任意地址上。由于字段“four_byte”的对齐值为 4，struct 中的字段也必须遵守 4 字节边界。这意味着 struct 中任意两个字段之间，字节数都必须是 4 的倍数。由于“one_byte”只占 1 字节，因此要在它之后插入 3 个填充字节，才能满足这个边界要求。

只要可能，相邻的字段就会被分到一组，以尽量减少所需的填充字节数：
...
这意味着，改变 struct 中字段的顺序，有时会导致它的大小不同：
...
在用汇编访问 struct 时，一定要考虑到任何可能的填充字节。

## 调用约定

一般来说，值传入和传出函数的方式都符合人们的预期：

- 整数类型的值，包括内存地址和枚举，通过通用寄存器（GPR）传递。
- 浮点数通过`xmm`寄存器传递。

不过，struct 是一种特殊情况。通常来说，struct 中的所有字段在内存中都占据相邻的空间，只要可能就应该放进同一个寄存器：
...
如果 struct 既有整数值的字段，又有浮点数的字段，那么 struct 中每个连续的 8 字节都应该单独分类：
...
如果整数和浮点数位于同一个 8 字节内，那么类型算作整数，整个 8 字节（包括浮点数）都会被放进一个 GPR：
...
~~~~exercism/caution
使用`cvtss2si`或`cvtsd2si`把浮点数转换为整数时，底层字节的组织方式会发生改变。

你可以用`movd`（用于 32 位值）和`movq`（用于 64 位值）把原始字节从`xmm`寄存器移动到 GPR，而不改变位表示。
~~~~

不过，大小超过 16 字节的 struct 通常会被放进内存，无论每个 8 字节是什么类型：
...
当 struct 作为实参传给函数时，它会占用所需数量的寄存器。如果它被放进内存，则改用栈。

类似的规则也适用于从函数返回的 struct。它们通常首先通过`rax`和`rdx`（如果是整数值）或`xmm0`和`xmm1`（如果是浮点数）返回。不过，如果它被放进内存，调用方必须为它预留空间，并把这个内存位置的地址作为隐含的第一个实参传给函数（放在`rdi`中）。然后函数把 struct 存入这块空间，并在`rax`中返回它的地址。
