...
Swift 提供了两大类数字：整数和浮点数。

- [整数][integers]表示没有小数部分的数，例如 `0`、`1`、`-1`、`42` 和 `-273`。
- [浮点数][floatingpoint]表示带有小数部分的数，例如 `0.0`、`3.14` 和 `-1.36969e-10`。

你可以在数字字面量中插入下划线来提高可读性：`1_000_000` 和 `1000000` 完全相同。
浮点字面量可以用十进制或指数记法书写。
...
Swift 提供了标准的[算术运算符][arithmetic-operators]用于数值计算：
...
你不能直接把 `Int` 和 `Double` 相加或相乘；必须先转换其中一个类型，使两者一致。
~~~~
...
`0.0`除以`0.0`会产生`nan`（Not a Number）。
...
[求余运算符][remainder-operator]（`%`）计算两个整数相除后剩下的余数：
...
你可以使用`rounded()`方法对浮点数取整。
默认情况下，`rounded()`会取最接近的整数。
你也可以指定具体的[舍入规则][rounding-rule]，例如`.up`或`.down`：
...
当你不加类型标注声明数字常量或变量时，Swift 会推断它的类型：
