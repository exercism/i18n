...
As anotações de tipo de dicionários são escritas assim:
...
Os dicionários podem ser criados assim:
...
Os itens podem ser recuperados com `get`.
Como uma chave pode estar ou não presente no dicionário, o resultado vem envolvido em um `Maybe`.
...
Os itens podem ser adicionados com `insert`.
...
Os itens podem ser atualizados com `update`.
Se a chave não existir, a função de atualização é chamada com `Nothing`.
...
Os itens podem ser removidos com `remove`.
Se a chave não existir, nada muda.
...
Os itens podem ser convertidos em uma lista com `toList`.
...
Os itens podem ser transformados com `map`.
...
Os dicionários podem ser combinados ou transformados com `merge`.
...
Qualquer função ou operador que pareça modificar um dicionário (como adicionar um elemento) na verdade retorna um novo dicionário.
O desempenho normalmente não é problema, porque a implementação evita alocações e cópias desnecessárias.

Em Elm, em geral é melhor usar abstrações de nível mais alto, como `Dict.map`, `Dict.filter` e `Dict.merge`, em vez de abstrações de nível mais baixo, como `Dict.get` e `Dict.remove`, embora isso dependa do contexto, é claro.
