...
O pacote [math/rand][mathrand] dá suporte à geração de números pseudoaleatórios.
...
Dada uma "semente" específica, os resultados são totalmente determinísticos.

No Go 1.20+, a semente é escolhida automaticamente ao acaso, então você verá uma sequência diferente de números aleatórios a cada vez que rodar seu programa.

Em versões anteriores do Go, a semente era `1` por padrão.
Então, para obter sequências diferentes nas diversas execuções do programa, você precisava definir a semente do gerador de números aleatórios manualmente, por exemplo, com a hora atual, antes de obter qualquer número aleatório.
