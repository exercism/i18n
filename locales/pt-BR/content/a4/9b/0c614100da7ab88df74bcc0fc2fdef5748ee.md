...
- `union(A, B)` ou `A ∪ B` (`\cup`): todas as entradas em A ou B ou em ambos.
...
- `issubset(A, B)` ou `A ⊆ B` (`\subseteq`) ou `B ⊇ A` (`\supseteq`): `true` se todas as entradas em A também estiverem em B.
- `issetequal(A, B)` (sem símbolo): `true` se A e B contiverem exatamente as mesmas entradas.
- `isdisjoint(A, B)` (sem símbolo): `true` se A e B não tiverem entradas em comum (ou seja, a interseção é vazia).
