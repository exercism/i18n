...
Um endereço de função que é passado adiante como um valor é chamado de [thunk][thunk].
Os thunks são um bloco de construção da [programação de ordem superior][higher-order] em assembly: código que opera sobre outro código.
...
`save_op` grava o endereço de função que recebe em `cached_fn`.
O valor persiste depois que `save_op` retorna, então qualquer chamada posterior a `apply_op` salta em cauda para o endereço que foi armazenado por último.
Isso torna possível mudar qual função `apply_op` invoca em tempo de execução.

## Tabelas de despacho

Armazenar endereços de função em um array torna possível selecionar funções diferentes de acordo com um índice, que pode depender de uma condição de tempo de execução.
Isso é chamado de [tabela de despacho][dispatch-table]:
...
Seu resultado pode depender de mais do que apenas os seus argumentos.
...
Muitos dos padrões comuns em linguagens de alto nível, como callbacks, métodos virtuais, geradores, currying, composição de funções e muitos outros, são construídos sobre thunks combinados com estado persistente.
