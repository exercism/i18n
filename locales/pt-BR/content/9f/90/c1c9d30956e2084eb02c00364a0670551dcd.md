...
O pacote [math/rand][mathrand] oferece suporte para gerar números pseudoaleatórios.
...
Também há suporte para embaralhar uma slice (ou outras estruturas de dados):
...
No Go 1.20+, a semente é escolhida automaticamente de forma aleatória, então você verá uma sequência diferente de números aleatórios cada vez que rodar seu programa.

Em versões anteriores do Go, a semente era `1` por padrão.
Então, para obter sequências diferentes em várias execuções do programa, você precisava definir manualmente a semente do gerador de números aleatórios, por exemplo, com a hora atual, antes de obter qualquer número aleatório.
