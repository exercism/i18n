...
O objeto [`Promise`][promise-docs] representa a conclusão (ou falha) eventual de uma operação assíncrona e o valor resultante dela.

<!-- prettier-ignore -->
~~~exercism/note
Este é um tópico difícil para muitas pessoas, especialmente se você programa em uma linguagem completamente _síncrona_.
Se você se sentir sobrecarregado, ou quiser aprender mais sobre **concorrência** e **paralelismo**, [assista (via go.dev)][talk-blog] ou [assista diretamente pelo vimeo][talk-video] e [leia os slides][talk-slides] da brilhante palestra "Concurrency is not parallelism".
...
Quando é criada, uma promise está pendente.
Em algum momento no futuro, ela pode ser _resolvida_ ou _rejeitada_.
Depois que uma promise é resolvida ou rejeitada uma vez, ela nunca mais pode ser resolvida ou rejeitada, nem seu estado pode mudar.

Em outras palavras:

1. Quando pendente, uma promise:
   - pode passar a estar resolvida ou rejeitada.
...
## Encadeando uma promise

Uma promise pode ser _continuada_ com uma ação futura assim que for resolvida ou rejeitada.

- [`promise.then()`][promise-then] é chamado assim que `promise` é resolvida
- [`promise.catch()`][promise-catch] é chamado assim que `promise` é rejeitada
- [`promise.finally()`][promise-finally] é chamado assim que `promise` é resolvida ou rejeitada

### **then**

Toda promise é "thenable".
Isso significa que existe uma função `then` disponível que será executada assim que a promise original for resolvida.
...
Isso vai registrar `"Success!"` depois de aproximadamente 1000 ms.
O estado e o valor de `promise1` serão `resolved` e `"Success!"`.
O estado e o valor de `promise2` serão `resolved` e `true`.

Há um segundo argumento disponível que é executado quando a promise original é rejeitada.
...
- Em cerca de 1/2 dos casos, isso vai registrar `"Success!"` depois de aproximadamente 1000 ms.
  - O estado e o valor de `promise1` serão `resolved` e `"Success!"`.
  - O estado e o valor de `promise2` serão `resolved` e `true`.
- Em cerca de 1/2 dos casos, isso vai registrar imediatamente `"NOPE!"`.
  - O estado e o valor de `promise1` serão `rejected` e `Nope!`.
  - O estado e o valor de `promise2` serão `resolved` e `false`.

É importante entender que, por causa das regras do ciclo de vida, quando ela faz `reject`, o `resolve` que chega cerca de ~1000ms depois é ignorado silenciosamente, já que o estado interno não pode mudar depois que ela foi rejeitada ou resolvida.
É importante entender que retornar um valor de uma promise a resolve, e lançar um valor a rejeita.
Quando `promise1` é resolvida e há um `onResolved` encadeado, `then(onResolved)`, esse seguimento é uma nova promise que pode ser resolvida ou rejeitada.
Quando `promise1` é rejeitada, mas há um `onRejected` encadeado, `then(, onRejected)`, esse seguimento é uma nova promise que pode ser resolvida ou rejeitada.

### **catch**

Às vezes você quer capturar erros e só continuar quando a promise original faz `reject`.
...
Em cerca de 1/2 dos casos, isso vai registrar `"Success!"` depois de aproximadamente 1000 ms.
Nos outros 1/2 dos casos, isso vai registrar imediatamente `42`.

- Se `promise1` for resolvida, `catch` é ignorado e o fluxo chega a `then`, e registra o valor.
  - O estado e o valor de `promise1` serão `resolved` e `"Success!"`.
  - O estado e o valor de `promise2` serão `resolved` e `"done"`;
- Se `promise1` for rejeitada, `catch` é executado, o que _retorna um valor_, e assim a cadeia agora está `resolved`, e o fluxo chega a `then`, e registra o valor.
  - O estado e o valor de `promise1` serão `rejected` e `"Nope!"`.
  - O estado e o valor de `promise2` serão `resolved` e `"done"`;

### **finally**

Às vezes você quer executar um código depois que uma promise é finalizada, independentemente de ela ser resolvida ou rejeitada.
...
- Nos casos em que `randomNumber` é `4-5`:
  ...
  - Haverá um registro:
    - `Promise completed`
    - _em alguns ambientes_ isso vai gerar um registro `"uncaught rejected promise: Error('Sampling did not result in a sample')"`
