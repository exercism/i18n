...
Para entender referências, é necessário entender ownership. Cada valor tem um único dono. Quando o escopo dele termina, o valor é descartado.
Por exemplo:
...
Uma [referência][borrow] representa um _empréstimo_ de um valor que tem dono. Em vez de transferir o ownership de `my_string`, podemos fazer com que `log` pegue `my_string` emprestado e depois o devolva para `my_string` em `main` ao final de `log`.

Para fazer isso, definimos `msg` como uma referência prefixando-a com um ampersand. O tipo `&String` é lido como "referência a uma `String`".

Como o argumento de `log` agora é uma referência, precisamos passar uma referência. Para obter uma referência a `my_string`, prefixe-a com um ampersand, que é o [operador de empréstimo](https://doc.rust-lang.org/reference/expressions/operator-expr.html#borrow-operators). Isso empresta `my_string` para `log` durante a execução dessa função.
...
A qualquer momento, um valor pode ter várias referências a ele, desde que todas sejam imutáveis, como mostrado abaixo
...
## Referências podem ser desreferenciadas

Às vezes, uma referência precisa ser desreferenciada para acessar o seu valor.
...
Para acessar o valor, o operador de desreferência (`*`) é colocado antes da referência, como mostrado abaixo
...
## Referências podem ser mutáveis

Referências podem se tornar mutáveis acrescentando `mut` ao ampersand, como mostrado abaixo
...
A qualquer momento, um valor pode ter uma referência mutável ou qualquer número de referências imutáveis a ele, como mostrado abaixo
...
Isso funciona, porque o compilador sabe que os empréstimos mutáveis não se sobrepõem
