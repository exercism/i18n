...
*Combinators* são palavras de ordem superior que capturam, em forma nomeada, padrões comuns de reorganização da pilha. Recorrer a eles significa escrever muito menos `dup`, `swap`, `over` e `rot`.
...
`keep`, `dip`, `bi`, `bi@` e `bi*` estão em [`kernel`][kernel]; `cleave` e as variantes mais longas estão em [`combinators`][combinators].
