...
Agora é hora de entrar nos detalhes técnicos.
O trecho de código abaixo mostra os termos mais comuns para consulta rápida.
...
É recomendável reservar esse padrão de maiúsculas para macros, caso não haja outra convenção.
~~~~

Se você tentar alterar uma variável constante depois de definida, seu código não vai compilar.
Isso ajuda a evitar alterações indesejadas, mas também abre possibilidades de otimização para o compilador.
Como pessoa, também fica mais fácil raciocinar sobre o código quando você sabe que certas partes não serão afetadas.
...
Isso se chama sobrecarga de funções e geralmente é feito quando essas funções realizam tarefas muito parecidas.
...
Algumas funções podem ficar bem longas, e muitas das chamadas delas podem usar os mesmos valores para a maioria dos parâmetros.
...
Se um parâmetro tem uma declaração padrão, todos os parâmetros à direita dele também precisam de uma declaração padrão.
Às vezes, sobrecargas de funções complicadas podem ser refatoradas para menos funções com argumentos padrão, o que melhora a facilidade de manutenção.
