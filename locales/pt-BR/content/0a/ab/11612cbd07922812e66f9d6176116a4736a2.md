...
Em certos casos, no entanto, o parâmetro de tipo aparece apenas no lado esquerdo da equação.
...
É possível somar dois `Distance Meter`, mas não é possível somar um `Distance Meter` com um `Distance Foot`.
...
## Registros extensíveis como tipos fantasma

Não há restrição sobre quais tipos podem ser fantasma, e tipos de registro também podem ser usados.
Os registros de Elm são [extensíveis][extensible-records], o que significa que podem conter mais campos do que declaram.
Quando usados como tipos fantasma, os registros conseguem expressar restrições complexas que podem se transformar através de funções.

Vamos adicionar uma nova unidade de distância dentro do módulo `Distance`, chamada `LegoBlock`.
Blocos físicos, sendo objetos regulamentados, sempre têm duas propriedades: eles têm distâncias não fracionárias e não negativas.
...
`nonFractional` e `nonNegative` são os campos de um registro que não existe fora de um argumento de tipo, então é adequado dar a eles o tipo `()`, chamado de __tipo unit__, que não guarda nenhuma informação além do fato de estar ali.

Obter distâncias `LegoBlock` arbitrárias é, claro, possível, por exemplo depois de calcular diferenças ou razões de distâncias.
...
Vamos adicionar algumas funções para que seja possível criar e refinar distâncias `LegoBlock`.
...
Essa é a força da técnica do tipo fantasma: oferecer escolhas flexíveis a quem usa, mantendo garantias fortes.
