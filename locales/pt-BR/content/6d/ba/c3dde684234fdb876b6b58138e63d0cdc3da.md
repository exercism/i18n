...
Quando usados como tipos fantasma, os registros conseguem expressar restrições complexas que podem se transformar por meio de funções.
...
`nonFractional` e `nonNegative` são campos de um registro que não existe fora de um argumento de tipo, então é adequado dar a eles o tipo `()`, chamado de __tipo unit__, que não carrega nenhuma informação além do fato de estar ali.
