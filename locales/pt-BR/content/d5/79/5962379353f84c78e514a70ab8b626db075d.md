...
O `jq` incorpora a biblioteca de regex [Oniguruma][oniguruma], que é amplamente compatível com as regexes do Perl v5.8.
...
Por exemplo, a classe de caractere de dígito (`\d`) deve ser escrita como `"\\d"`.
~~~~
...
- nada, se não houve correspondência, ou
- um objeto contendo várias propriedades, se houve correspondência.

Este exemplo procura duas vogais consecutivas idênticas usando a sintaxe de retro-referência, `\1`.
...
Semelhante ao filtro `match`, o filtro `capture` retorna um objeto se houve correspondência.
...
Para usar uma regex com `split`, você **precisa** fornecer o 2º argumento; não tem problema usar uma string vazia.
~~~~
...
O texto de substituição pode se referir às substrings correspondentes; use capturas nomeadas e [interpolação de strings][jq-interp].
...
Em todos os filtros acima, FLAGS é uma string composta de zero ou mais das flags compatíveis.
...
- `p` - Os modos s e m são ativados
- `s` - Modo de linha única ('^' -> '\A', '$' -> '\Z')
- `l` - Encontra as correspondências mais longas possíveis
- `x` - Formato de regex estendido (ignora espaços em branco e comentários)
