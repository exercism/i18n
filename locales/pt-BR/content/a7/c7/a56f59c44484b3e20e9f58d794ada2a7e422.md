...
[Valores padrão de parâmetros][default-parameter-values] podem ser fornecidos para qualquer um dos parâmetros de uma função, atribuindo um valor ao parâmetro na lista de parâmetros, logo após a anotação de tipo do parâmetro.
Quando um valor padrão é especificado, quem chama a função pode omitir esse parâmetro ao chamá-la, e o valor padrão será usado no lugar.
