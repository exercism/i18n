# Instruções

Neste exercício, você vai processar linhas de log.

Cada linha de log é uma string formatada assim: `"[<LEVEL>]: <MESSAGE>"`.

Há três níveis de log diferentes:

- `INFO`
- `WARNING`
- `ERROR`

Você tem três tarefas. Cada uma recebe uma linha de log e pede que você faça algo com ela.

## 1. Obtenha a mensagem de uma linha de log

Implemente a função `message` para retornar a mensagem de uma linha de log:

```clojure
(message "[ERROR]: Invalid operation")
;;=> "Invalid operation"
```

Qualquer espaço em branco no início ou no fim deve ser removido:

```clojure
(message "[WARNING]:  Disk almost full\r\n")
;;=> "Disk almost full"
```

## 2. Obtenha o nível de log de uma linha de log

Implemente a função `log-level` para retornar o nível de log de uma linha de log, que deve ser retornado em minúsculas:

```clojure
(log-level "[ERROR]: Invalid operation")
;;=> "error"
```

## 3. Reformate uma linha de log

Implemente a função `reformat` que reformata a linha de log, colocando a mensagem primeiro e o nível de log depois dela entre parênteses:

```clojure
(reformat "[INFO]: Operation completed")
;;=> "Operation completed (info)"
```
