...
Behaviours permitem definir interfaces (conjuntos de funções e macros) em um _módulo de behaviour_ que depois podem ser implementados por diferentes _módulos de callback_. Graças à interface compartilhada, esses módulos de callback podem ser usados de forma intercambiável.
...
Para marcar qual função vem de qual behaviour, devemos usar o atributo de módulo `@impl` antes de cada função. Seu valor deve ser o nome do módulo de behaviour que define esse callback.
...
Ao definir um behaviour, é possível fornecer uma implementação padrão de um callback. Essa implementação deve ser definida na quoted expression da macro `__using__/1`. Para que seja possível para quem usa o módulo de behaviour substituir a implementação padrão, chame a macro `defoverridable/1` depois da implementação da função. Ela aceita uma lista de palavras-chave com nomes de funções como chaves e aridades de função como valores.
...
Repare que definir funções dentro de `__using__/1` é desaconselhado para qualquer finalidade que não seja definir implementações padrão de callbacks, mas você sempre pode definir funções em outro módulo e importá-las na macro `__using__/1`.
