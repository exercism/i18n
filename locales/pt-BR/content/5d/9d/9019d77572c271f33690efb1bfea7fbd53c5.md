...
Em JavaScript, as instâncias de um [`Array`][arrays-concept] são objetos comuns, decorados com vários [métodos úteis][array-docs].
Ele inclui a propriedade `length`, além de métodos para percorrer, analisar e modificar o array.
Os elementos do array são propriedades indexadas desse array.
Em vez de iterar manualmente sobre essas propriedades indexadas, use a análise de arrays.
Muitas das funções nativas que analisam o conteúdo de um array recebem como argumento uma função que retorna true ou false. Uma função assim é chamada de [`predicate`][predicate_in_programming].

Em geral, usar as abstrações e os métodos nativos é mais idiomático, legível e fácil de manter do que usar um `for` ou um `.forEach` equivalente.
