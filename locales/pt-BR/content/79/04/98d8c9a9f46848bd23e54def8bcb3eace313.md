...
Thunks são um bloco de construção da **programação de ordem superior** em assembly: código que opera sobre outros códigos.
...
`save_op` escreve o endereço de função que recebe em `cached_fn`.
O valor persiste depois que `save_op` retorna, então qualquer chamada posterior a `apply_op` faz um salto de cauda para o endereço armazenado por último.
...
`tick` invoca a função dada com a contagem atual como argumento, e então avança a contagem.
Assim, uma primeira chamada `tick(square)` invoca `square(0)`, a próxima chamada `tick(square)` invoca `square(1)`, a seguinte `square(2)`, e assim por diante.
...
Quando `invoke` é chamado, ele executa a função capturada com o argumento salvo.
