...
A desestruturação pode vincular valores em vínculos `let`, em argumentos de função e, claro, em expressões case, desde que haja apenas um formato possível para os dados.
...
A desestruturação também pode ser usada com [records][records-pattern-matching] ou com a [palavra-chave `as`][as-keyword].
