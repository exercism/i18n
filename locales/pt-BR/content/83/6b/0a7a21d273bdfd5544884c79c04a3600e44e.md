...
O `jq` incorpora a biblioteca de regex [Oniguruma][oniguruma], que é em grande parte compatível com as regexes do Perl v5.8.
...
Esse filtro retorna um resultado _boolean_.
...
`scan` produz um _fluxo_ de substrings.
...
# => ["first   second           third fourth"]
```
Só um resultado: a _string fixa_ `\s+` não foi encontrada na entrada.

O `split` de aridade 1 não consegue lidar com espaços em branco _arbitrários_.
...
- `g` - Busca global (encontra todas as correspondências, não só a primeira)
...
- `p` - Habilita os modos s e m ao mesmo tempo
