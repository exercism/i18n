...
Усі ключі в словнику мають бути одного типу, і всі значення також мають бути того самого типу.
...
Типи словників можна записати явно одним із двох способів: `Dictionary<Key, Value>` або скороченим синтаксисом `[Key: Value]`.
...
До значень можна звертатися за допомогою індексної нотації: ключ ставлять у квадратні дужки після назви словника.
Оскільки ключа в словнику може не бути, звернення за індексом повертає *опціональне* значення (`Value?`).
Якщо ключ є, Swift повертає розгорнуте значення; якщо ні, повертає `nil`:
...
Щоб не працювати з опціональними значеннями, можна вказати типове значення за допомогою параметра `default`:
...
## Змінювання словників

Для змінних словників, оголошених за допомогою `var`, можна додавати пари ключ-значення або оновлювати їх за допомогою присвоєння за індексом:
...
Оскільки словники невпорядковані, сортування дає масив пар ключ-значення.
