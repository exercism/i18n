...
[`Dict`][dict] в Elm - це незмінний словник, який містить нуль або більше пар ключ-значення.

Зі словниками працюють за допомогою функцій та операторів, визначених у [модулі `Dict`][dict-module].

Анотації типів для словників записують так:
...
Елементи можна дістати за допомогою `get`.
Оскільки ключ може бути присутній у словнику, а може й ні, результат буде загорнуто в `Maybe`.
...
Елементи можна додавати за допомогою `insert`.
Елементи для ключів, які вже є у словнику, замінюються.
...
Елементи можна оновлювати за допомогою `update`.
...
Елементи можна видаляти за допомогою `remove`.
...
Елементи можна перетворювати за допомогою `map`.
...
Будь-яка функція чи оператор, які нібито змінюють словник (наприклад, додають елемент), насправді повертають новий словник.
Продуктивність зазвичай не є проблемою, оскільки реалізація не виконує зайвих виділень памʼяті та копіювань.
