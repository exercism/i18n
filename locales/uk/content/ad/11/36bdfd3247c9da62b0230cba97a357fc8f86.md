...
У Elm [`Dict`][dict] - це незмінний словник із нуля або більше пар ключ-значення.
...
Оскільки ключ може бути присутнім у словнику, а може й ні, результат буде загорнуто в `Maybe`.
...
Продуктивність зазвичай не є проблемою, адже реалізація запобігає зайвим виділенням памʼяті та копіюванням.

В Elm зазвичай краще використовувати абстракції вищого рівня, як-от `Dict.map`, `Dict.filter` і `Dict.merge`, замість абстракцій нижчого рівня, як-от `Dict.get` і `Dict.remove`, хоча це, звісно, залежить від контексту.
