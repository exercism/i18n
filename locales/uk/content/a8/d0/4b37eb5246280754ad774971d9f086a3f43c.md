...
Кожен трек повинен мати каталог `docs` з такими файлами:
...
**Призначення:** Короткий вступ до мови.

**Показується на:** Сторінці треку, за умови, що студент ще не приєднався до треку.
...
Перегляньте [цю сторінку](https://exercism.org/tracks/fsharp), щоб побачити, як це виглядає після рендерингу (зауважте: побачити це можна, лише якщо до треку ще не приєдналися).

## Файл: `INSTALLATION.md`

**Призначення:** Описати, що студентові потрібно встановити, щоб працювати над треком на власній системі через CLI.

**Показується на:** Сторінці документації треку за адресою `https://exercism.org/docs/tracks/<track>/installation`.
...
Перегляньте [цю сторінку](https://exercism.org/docs/tracks/fsharp/installation), щоб побачити, як це виглядає після рендерингу.

## Файл: `LEARNING.md`

**Призначення:** Посилання на навчальні ресурси.

**Показується на:** Сторінці документації треку за адресою `https://exercism.org/docs/tracks/<track>/learning`.
...
Перегляньте [цю сторінку](https://exercism.org/docs/tracks/fsharp/learning), щоб побачити, як це виглядає після рендерингу.

## Файл: `SNIPPET.txt`

**Призначення:** Дати візуальне уявлення про те, як виглядає синтаксис мови.

**Показується на:** Сторінці треку, за умови, що студент ще не приєднався до треку.

Сніпет має бути відносно невеликим (приблизно 10 рядків).
...
## Файл: `REPRESENTER_NORMALIZATIONS.md`

**Призначення:** Перелічити нормалізації, які репрезентер застосовує до рішення.
...
**Призначення:** Посилання на корисні ресурси.

**Показується на:** Сторінці документації треку за адресою `https://exercism.org/docs/tracks/<track>/resources`.
...
Перегляньте [цю сторінку](https://exercism.org/docs/tracks/fsharp/resources), щоб побачити, як це виглядає після рендерингу.

## Файл: `TESTS.md`

**Призначення:** Описати все, що стосується запуску тестів на треку.

**Показується на:** Сторінці документації треку за адресою `https://exercism.org/docs/tracks/<track>/tests`.
...
Перегляньте [цю сторінку](https://exercism.org/docs/tracks/fsharp/tests), щоб побачити, як це виглядає після рендерингу.

## Файл: `config.json`

**Призначення:** Повідомити сайту, які сторінки доступні.

**Показується на:** (не показується)
...
За бажанням можна додати додаткові документи.
Щоб це зробити, потрібно виконати дві дії:

1. Створити документ у каталозі `docs` (наприклад, `docs/PACKAGES.md`)
2. Додати запис про новий документ у файл `docs/config.json`.
   Наприклад:
...
Після злиття в `main` документ можна знайти за адресою `https://exercism.org/docs/tracks/<track_slug>/<document_slug>` (наприклад, `https://exercism.org/docs/tracks/fsharp/packages`)
