...
Veamos un ejemplo de cómo escribir un parser para (un subconjunto de) el lenguaje de programación [LOLCODE][lolcode].
Considera el siguiente programa comentado:
...
`versionP` se construye usando un _pipeline de parsers_.
...
Ya tenemos un parser para cada comando.
Claro que, en un programa general, no se puede predecir el orden de estos comandos, así que construimos un parser de comandos genérico
...
Por último, a veces necesitamos decidir si hacemos fallar un parser según su contenido ya parseado.
Por ejemplo, si necesitáramos un parser que hiciera fallar los programas que empiezan con dos comandos «HAI» seguidos, podríamos usar
