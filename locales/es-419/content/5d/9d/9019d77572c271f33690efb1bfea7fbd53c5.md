...
En JavaScript, las instancias de [`Array`][arrays-concept] son objetos normales, adornados con muchos [métodos útiles][array-docs]. Incluye la propiedad `length`, además de métodos para recorrer, analizar y mutar el array. Los elementos del array son propiedades indexadas de ese array. En vez de iterar manualmente sobre esas propiedades indexadas, usa el análisis de arrays. Muchas de las funciones integradas que analizan el contenido de un array reciben como argumento una función que devuelve true o false. A esa función se le llama [`predicate`][predicate_in_programming].

En general, usar las abstracciones o los métodos integrados es más idiomático, legible y fácil de mantener que usar un `for` o su equivalente con `.forEach`.
