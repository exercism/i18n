# Anexo de instrucciones

## Instrucciones especiales para Python

Para este ejercicio **no se requiere** una biblioteca de terceros. Ignora las instrucciones sobre la **biblioteca FRP**.


## Mensajes de excepción

A veces es necesario [lanzar una excepción](https://docs.python.org/3/tutorial/errors.html#raising-exceptions). Cuando lo hagas, siempre debes incluir un **mensaje de error significativo** que indique cuál es el origen del error. Esto hace que tu código sea más legible y facilita mucho la depuración. En los casos en que sepas que el origen del error será de cierto tipo, puedes optar por lanzar uno de los [tipos de error integrados](https://docs.python.org/3/library/exceptions.html#base-classes), pero aun así debes incluir un mensaje significativo.

Este ejercicio en particular requiere que uses la [sentencia raise](https://docs.python.org/3/reference/simple_stmts.html#the-raise-statement) para «lanzar» un `ValueError` cuando el juego haya terminado pero el jugador intente seguir jugando. Las pruebas solo pasarán si haces `raise` a la `exception` e incluyes un mensaje con ella.

Para lanzar un `ValueError` con un mensaje, escribe el mensaje como argumento del tipo `exception`:

```python
# when player tries to play, but the game is already over.
raise ValueError("The game has already ended.")
```
