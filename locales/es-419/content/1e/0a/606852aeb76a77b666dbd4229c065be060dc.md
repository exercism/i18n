...
El concepto [`Basics`][basics] presentó dos formas de definir una [función][functions].
...
En este caso, `x -> 2x` es una [«función anónima»][anonymous-function].
...
Las funciones anónimas son comunes en el código Julia, sobre todo cuando se combinan con [funciones de orden superior][HOF] como `map()` y `filter()`.

## Argumentos de las funciones

Hasta ahora en el temario solo hemos visto funciones que tienen un número preciso de argumentos y que exigen que las llamadas a la función los proporcionen todos, en el orden correcto.
Esto sería restrictivo e incómodo, así que hay varias otras opciones.
...
El splat se usa en las _llamadas_ a funciones para expandir colecciones en los valores individuales que la función necesita.
...
El slurp se usa en la _definición_ de una función para empaquetar una cantidad arbitraria de valores individuales en una colección.
...
La definición de la función decide a qué palabras clave responder y cuáles ignorar.
