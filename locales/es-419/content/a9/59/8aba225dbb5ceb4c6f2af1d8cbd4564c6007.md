...
A menudo, se accede al contenido de un `Option` mediante coincidencia de patrones.
...
El módulo `gleam/option` también define varias funciones útiles para trabajar con tipos `Option`, como `unwrap`, que devuelve el contenido de un `Option` o un valor predeterminado si es `None`.
