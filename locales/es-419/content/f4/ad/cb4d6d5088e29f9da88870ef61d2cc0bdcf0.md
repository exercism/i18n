...
Los dicts se manipulan con funciones y operadores definidos en el [módulo `Dict`][dict-module].

Las anotaciones de tipo para los dicts se escriben así
...
Los elementos se pueden obtener con `get`.
Como una clave puede estar o no presente en el dict, el resultado quedará envuelto en un `Maybe`.
...
Los elementos se pueden agregar con `insert`.
Los elementos de claves que ya están en el dict se reemplazan.
...
Los elementos se pueden actualizar con `update`.
Si la clave no existe, se llamará a la función de actualización con `Nothing`.
...
Los elementos se pueden eliminar con `remove`.
...
Una lista de pares clave-valor se puede convertir en un dict con `fromList`.
...
Los elementos se pueden convertir en una lista con `toList`.
...
Se pueden eliminar varios elementos con `filter`.
...
Los elementos se pueden transformar con `map`.
...
Los dicts se pueden combinar y transformar con `merge`.
...
Cualquier función u operador que parezca modificar un dict (como agregar un elemento) en realidad devolverá un dict nuevo.
Por lo general, el rendimiento no es un problema, ya que la implementación evita asignaciones y copias innecesarias.

En Elm, por lo general es mejor usar abstracciones de más alto nivel, como `Dict.map`, `Dict.filter` y `Dict.merge`, en lugar de abstracciones de más bajo nivel como `Dict.get` y `Dict.remove`, aunque, por supuesto, depende del contexto.
