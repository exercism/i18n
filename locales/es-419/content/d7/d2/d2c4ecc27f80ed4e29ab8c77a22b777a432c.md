...
Se puede acceder a la información de fecha y hora, compararla y manipularla a través de sus métodos, pero también hay algunas funciones que se llaman en el propio paquete `time`.
La fecha y hora actuales se pueden obtener con la función [`time.Now`][now].

La función [`time.Parse`][parse] convierte strings en valores de tipo `Time`.
Go tiene una forma especial de definir el formato que esperas para la conversión.
...
El método [`Time.Format()`][format] devuelve una representación en string del tiempo.
Igual que con la función `Parse`, el formato de destino también se define mediante un ejemplo que usa los valores de la marca de tiempo especial.
...
En Go también hay [constantes de formato][const] de fecha y marca de tiempo predefinidas.
...
El tipo `time.Time` tiene varios métodos para acceder a un momento concreto. Por ejemplo, hora: [`Time.Hour()`][hour], mes: [`Time.Month()`][month].
Puedes encontrar más información sobre cómo funciona esto en la [documentación oficial][time].

El paquete [`time`][time] incluye otro tipo, [`Duration`][duration], que representa el tiempo transcurrido, además de compatibilidad con ubicaciones y zonas horarias, temporizadores y otras funcionalidades relacionadas que se verán en otro concepto.
