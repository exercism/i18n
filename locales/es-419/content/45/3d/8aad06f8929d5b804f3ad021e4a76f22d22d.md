...
Un [`Dict`][dict] en Elm es un diccionario inmutable de cero o más pares de clave-valor.

Los dict se manipulan con funciones y operadores definidos en el [módulo `Dict`][dict-module].

Las anotaciones de tipo para los dict se escriben así:
...
Como una clave puede estar o no presente en el dict, el resultado se envuelve en un `Maybe`.
...
Si la clave no existe, se llama a la función de actualización con `Nothing`.
...
Una lista de pares de clave-valor se puede convertir en un dict con `fromList`.
...
Los dict se pueden combinar o transformar con `merge`.
...
Cualquier función u operador que parezca modificar un dict (como agregar un elemento) en realidad devuelve un dict nuevo.
