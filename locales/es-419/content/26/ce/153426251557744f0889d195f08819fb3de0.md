...
Los valores `a` y `m` deben ser _coprimos_ (o _primos entre sí_) para que el descifrado automático funcione, es decir, que su único factor común sea el número `1` (puedes encontrar más información en el [artículo de Wikipedia sobre números coprimos][coprime-integers]).
...
Para los fines de este ejercicio, los dígitos son válidos, pero no se cifran.
Los espacios y los caracteres de puntuación se excluyen.
El texto cifrado se escribe en grupos de longitud fija separados por espacios; el tamaño de grupo tradicional es de `5` letras.
Esto hace que sea más difícil adivinar el texto cifrado a partir de los límites entre palabras.
...
- es importante notar que `a^-1` es el inverso multiplicativo modular (IMM) de `a mod m`
- el inverso multiplicativo modular solo existe si `a` y `m` son coprimos.

El IMM de `a` es `x` tal que el residuo de dividir `ax` entre `m` sea `1`:

```text
ax mod m = 1
```

Puedes encontrar más información sobre cómo hallar un inverso multiplicativo modular y qué significa en el [artículo relacionado de Wikipedia][mmi].
