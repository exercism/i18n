...
Esto permite que el código sea más conciso y legible, y se hace separando con una coma las variables que se van a asignar, por ejemplo `first, second, third = [1, 2, 3]`.

El operador splat (`*`) y el operador splat doble (`**`) se usan con frecuencia en contextos de descomposición.
...
El operador splat doble (`**`) se puede usar para combinar varios **hashes** en un solo **hash** al _descomponer_ cada uno en un nuevo **hash** común.
Esta es una sintaxis que sirve para diferenciar entre varios valores aceptados como argumento posicional y un caso en el que aceptamos cualquier cantidad de argumentos de palabra clave.
...
A menudo verás estos argumentos definidos como `def some_method(*arguments, **keyword_arguments)` y usados como `some_method(*some_array, **some_hash)`.
...
~~~~exercism/note
Esto también se conoce como «Parallel Assignment» y se puede usar para evitar una variable temporal.
~~~~
...
Si hay más valores que variables, los valores adicionales se ignorarán:
...
Como los valores aparecen dentro de los **arrays** en un orden de índice, se desempaquetan en las variables en ese mismo orden:
...
Descomponer y asignar valores de **arrays** dentro de un **array** (_también conocido como array anidado_) funciona igual que una descomposición superficial, pero necesita una [expresión de descomposición delimitada (`()`)][delimited decomposition expression] para aclarar el contexto o la posición de los valores:
...
Si la descomposición tiene variables con una ubicación incorrecta o una cantidad incorrecta de valores, obtendrás un **error de sintaxis**:
...
El error de sintaxis no está ligado a la estructura de datos.

### Descomponer un array con el operador splat simple (`*`)

Cuando [descompones un **array**][decomposition] puedes usar el operador splat (`*`) para capturar los valores «sobrantes».
Esto es más claro que cortar el **array** (_que en algunas situaciones es menos legible_).
Por ejemplo, podemos extraer el primer elemento y luego asignar los valores restantes a un nuevo **array** sin ese primer elemento:
...
También podemos extraer los valores al principio y al final del **array** mientras agrupamos todos los valores de en medio:
...
También podemos usar `*` en la descomposición profunda:
...
Para poder desempaquetar un **hash**, primero debes convertirlo en un **array**.
...
Esto es útil cuando quieres _descomponer_ valores, hacer cambios y luego _componer_ los resultados de vuelta en una variable.
También hace posible combinar 2 o más **arrays**/**hashes**.

### Componer un array con el operador splat (`*`)

Componer un **array** se puede hacer con el operador splat (`*`).
Esto empaquetará todos los valores en un **array**.
...
### Componer un hash con el operador splat doble (`**`)

Componer un hash se hace con el operador splat doble (`**`).
Esto empaquetará todos los pares **clave**/**valor** de un hash en otro hash, o combinará dos hashes entre sí.
...
## Uso del operador splat (`*`) y del operador splat doble (`**`) con métodos

### Composición con parámetros de método
...
`*arguments` se usa para empaquetar una cantidad arbitraria de argumentos posicionales (sin palabra clave) y `**keyword_arguments` se usa para empaquetar una cantidad arbitraria de argumentos de palabra clave.
...
`*arguments` y `**keyword_arguments` también se pueden usar en combinación uno con otro:
...
~~~~exercism/caution
Los argumentos deben estructurarse en un orden específico:
...
Puedes usar el operador splat (`*`) para desempaquetar un **array** de argumentos en una llamada a método:
...
También puedes usar el operador splat doble (`**`) para desempaquetar un **hash** de argumentos en una llamada a método:
