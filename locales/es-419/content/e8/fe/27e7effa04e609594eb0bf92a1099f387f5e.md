...
Los tipos de Elm permiten tipos genéricos, que por lo general aportan flexibilidad a una interfaz.
...
Nota que en el ejemplo de arriba, el parámetro de tipo `a` se usa en ambos lados del signo igual `=`.
Decimos que `a` está ligado a algunos datos dentro de la definición del tipo.

## La técnica del tipo fantasma

En ciertos casos, sin embargo, el parámetro de tipo solo aparece en el lado izquierdo de la ecuación.

```elm
type Distance unit = Distance Float
```

En la definición de `Distance` de arriba, `unit` es un parámetro de tipo libre, no está ligado a ningún dato del tipo.
...
Por ejemplo, queremos asegurarnos de sumar solo distancias de la misma unidad.
...
Los usuarios pueden sumar dos `Distance Meter`, pero no pueden sumar una `Distance Meter` con una `Distance Foot`.
...
No hay ninguna restricción sobre qué tipos pueden ser fantasma, y los tipos de registro también se pueden usar.
Cuando se usan como tipos fantasma, los registros pueden expresar restricciones complejas que se transforman a través de funciones.

Agreguemos una nueva unidad de distancia dentro del módulo `Distance`, llamada `LegoBlock`.
Los bloques físicos, al ser objetos regulados, siempre tienen dos propiedades: sus distancias no son fraccionarias ni negativas.
...
`nonFractional` y `nonNegative` son los campos de un registro que no existe fuera de un argumento de tipo, así que es adecuado darles el tipo `()`, llamado el __tipo unidad__, que no contiene más información que el hecho de estar ahí.

Obtener distancias `LegoBlock` arbitrarias es, por supuesto, posible; por ejemplo, después de calcular diferencias o razones entre distancias.
...
Todos los valores de arriba son válidos y van a compilar; sin embargo, nos interesan especialmente valores como `fourStuds`, que tienen tanto la propiedad `nonFractional` como la `nonNegative`, porque representan bloques físicos que se pueden combinar con
...
Nota que `floorDistance`, `ceilingDistance` y `absDistance` pueden manejar unidades distintas de `LegoBlock` y, en general, no hacen ninguna suposición sobre las propiedades de entrada: solo garantizan que la salida tendrá una propiedad específica, ya sea `nonFractional` o `nonNegative`.
...
En general, `floorDistance` y `ceilingDistance` dan resultados distintos, pero las mismas garantías.
Esa es la fortaleza de la técnica del tipo fantasma: ofrecer a los usuarios opciones flexibles sin perder garantías sólidas.
