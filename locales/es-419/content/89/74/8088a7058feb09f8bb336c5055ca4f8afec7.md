...
Es hora de entrar en detalles técnicos.
...
C++ usa la palabra clave `const` como calificador de las constantes.
...
~~~~exercism/note
A menudo verás las constantes escritas en _UPPER_SNAKE_CASE_.
...
Como persona, también es más fácil razonar sobre el código si sabes que ciertas partes no se verán afectadas.
...
Varias funciones pueden tener el mismo nombre si la lista de parámetros es diferente.
...
El ejemplo de `play_sound` tiene seis sobrecargas diferentes para adaptarse a distintos escenarios:
...
Algunas funciones pueden volverse muy extensas y muchas de sus llamadas podrían usar los mismos valores para la mayoría de los parámetros.
...
Si un parámetro tiene un valor predeterminado, todos los parámetros a su derecha también deben tenerlo.
A veces las sobrecargas de funciones complicadas se pueden refactorizar a menos funciones con argumentos predeterminados para mejorar la mantenibilidad.
