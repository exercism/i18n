...
Los bloques y los procs se usan muchísimo en Crystal, por ejemplo para iteraciones y callbacks.
Pero también para crear [DSL (lenguajes específicos del dominio)][dsl]; algunos ejemplos de DSL son el framework web [Kemal][kemal] o el framework de pruebas [Spec][spec] (que es lo que se usa para probar tus soluciones en Crystal).
...
[`yield`][yield] es una palabra clave que ejecuta el bloque que se le pasó al método.
...
Como con otros tipos explícitos, el compilador hará que los tipos sean los esperados.
Si el bloque no coincide con el tipo que se le dio al método, se producirá un error.
