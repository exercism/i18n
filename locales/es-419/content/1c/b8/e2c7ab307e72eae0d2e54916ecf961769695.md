...
Piensa en sumar cinco a un valor en memoria:

```x86asm
add qword [rel counter], 5
```

Por debajo, el procesador no tiene una forma de sumar directamente a la memoria.
...
Este es un patrón común llamado **read-modify-write (RMW)**.
...
Normalmente no lo notamos porque, en un solo núcleo, se garantiza que cada instrucción produce su efecto por completo antes de la siguiente.
...
Cada hilo tiene sus propios registros y su propia pila, así que un valor que está en un registro, o una variable local en la pila de un hilo, es exclusivo de ese hilo y no puede sufrir una carrera.
...
Dos hilos pueden ejecutarse de forma concurrente en el mismo núcleo, intercalándose entre sí, o en paralelo en núcleos distintos.

Los hilos intercalados ya pueden competir en un read-modify-write si este se divide en varias instrucciones, ya que el sistema operativo puede cambiar de hilo entre dos instrucciones cualesquiera.
...
La verdadera atomicidad en _varios_ núcleos es lo que ofrecen las instrucciones de abajo.
~~~~
...
Como es habitual, se puede usar con dos operandos de registro o con un operando de memoria y un operando de registro:
...
~~~~exercism/caution
`xchg` es atómica automáticamente cuando uno de los operandos es una ubicación de memoria.
Esto también significa que la operación es mucho más lenta en esa situación.

Si no necesitas atomicidad, haz el intercambio mediante un registro libre con instrucciones `mov` normales.
~~~~
...
Esto significa que el núcleo retiene la memoria en exclusiva durante todo el proceso, así que ningún otro núcleo puede leer o escribir esa ubicación mientras tanto.
...
~~~~exercism/caution
Mantener una ubicación en exclusiva y dejar fuera a todos los demás núcleos no sale gratis.
Una operación con el prefijo `lock` es notablemente más lenta que su forma normal, y más lenta aún cuando varios núcleos compiten por la misma ubicación.

Este prefijo debería reservarse para la memoria que se espera que modifique más de un hilo.
Evita usarlo si la memoria no se comparte o si solo se lee.
~~~~

### Intercambio y suma

Un `lock add` normal actualiza la memoria, pero descarta el valor antiguo.
A menudo el valor antiguo es justo lo que se quiere, por ejemplo para dar a cada hilo un número de turno distinto.

La instrucción `xadd` (la `x` de intercambio) devuelve el valor anterior mientras suma.
...
Al igual que con `lock add`, el contador termina en el número exacto de llamadas.
Sin embargo, a diferencia de `lock add`, también se devuelve cada valor intermedio, uno a cada quien la llama.
...
Esa actualización condicional es lo que ofrece `cmpxchg`, comparación e intercambio, y es la más general de estas primitivas.

`cmpxchg dest, src` usa `rax` como acumulador implícito y lo compara con `dest`:

- Si `dest == rax`, entonces `dest = src` y `ZF = 1`.
- Si `dest != rax`, entonces `rax = dest` y `ZF = 0`.

Fíjate que `dest` solo se actualiza cuando es igual al valor esperado, que se cargó antes en `rax`.
Esta igualdad garantiza que `dest` todavía contiene el valor a partir del cual se calculó el nuevo, así que nunca se aplica una actualización basada en una lectura obsoleta.
Eso hace de `cmpxchg` el bloque de construcción de una actualización atómica, también conocida como **compare-and-swap (CAS)**:
...
Este **bucle de reintentos** es el corazón de las actualizaciones sin bloqueos.
La ventana entre la lectura y la comparación e intercambio es exactamente cuando otro hilo podría intervenir, y `cmpxchg` lo detecta al negarse a almacenar un valor calculado a partir de una lectura obsoleta.

### Ordenamiento de memoria

Todas las operaciones hasta ahora han tocado una sola ubicación.
Cuando los hilos se coordinan a través de más de una ubicación, aparece una nueva pregunta: ¿en qué orden se vuelven visibles las escrituras de un hilo para otro?
Las reglas que responden a esta pregunta son el **ordenamiento de memoria** del procesador.

El concepto de código sin ramas introdujo la idea de que un núcleo moderno no avanza trabajosamente por las instrucciones de una en una.
...
x86-64 mantiene un **ordenamiento de memoria fuerte** entre las cargas y los almacenamientos normales, de modo que en cada núcleo:
...
Una instrucción con el prefijo `lock`, o un `xchg` con un operando de memoria, es una barrera completa: nada parece cruzar de un lado a otro.
Por eso bastan para garantizar un ordenamiento completo en la mayoría de las situaciones.

### Espera activa y `pause`

Las instrucciones que ponen una bandera y además devuelven su estado anterior se conocen como **test-and-set**.
...
4. Si el valor devuelto es `0`, significa que el bloqueo estaba libre.
   `xchg` ya lo puso en `1`, y los demás núcleos esperarán hasta que este núcleo lo libere.
...
Así, la CPU puede reducir el consumo de energía del hilo que espera y ceder el paso a un hilo hermano que comparte el mismo núcleo.
...
No se necesita nada más que un `mov`, porque en x86-64 las cargas y los almacenamientos nunca se reordenan después de un almacenamiento posterior.
Se dice que un almacenamiento que nunca adelanta a los accesos anteriores tiene **ordenamiento de liberación**, y en x86 todo almacenamiento normal lo tiene.

~~~~exercism/note
Cualquier instrucción que pruebe y establezca atómicamente una ubicación de memoria puede usarse para un spinlock.
Por ejemplo, puede usarse `lock bts` en lugar de `xchg`, para establecer un bit específico mientras se comprueba si ya estaba establecido.

Fíjate que las banderas, como la `CF` que modifica `bts`, son parte de `rflags`, un registro.
