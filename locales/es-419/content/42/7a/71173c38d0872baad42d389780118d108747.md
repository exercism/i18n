...
Si te resulta abrumador, o quieres aprender más sobre la **concurrencia** y el **paralelismo**, [mira (en go.dev)][talk-blog] o [mira directamente en Vimeo][talk-video] y [lee las diapositivas][talk-slides] de la brillante charla «Concurrency is not parallelism».
...
Una vez que una promesa se resuelve o se rechaza, no puede volver a resolverse ni rechazarse, y su estado tampoco puede cambiar.

En otras palabras:

1. Mientras está pendiente, una promesa:
   - puede pasar al estado resuelta o rechazada.
2. Cuando está resuelta, una promesa:
...
En los ejemplos anteriores, `value` puede ser _cualquier cosa_, incluido un error, `undefined`, `null` u otra promesa.
Por lo general, querrás resolver con un valor que no sea un error.

## Cómo rechazar una promesa
...
En los ejemplos anteriores, `reason` puede ser _cualquier cosa_, incluido un error, `undefined` o `null`.
Por lo general, querrás rechazar con un error.

## Cómo encadenar una promesa

Una promesa puede _continuarse_ con una acción futura una vez que se resuelve o se rechaza.

- Se llama a [`promise.then()`][promise-then] una vez que `promise` se resuelve
- Se llama a [`promise.catch()`][promise-catch] una vez que `promise` se rechaza
- Se llama a [`promise.finally()`][promise-finally] una vez que `promise` se resuelve o se rechaza
...
- En aproximadamente la mitad de los casos, esto imprimirá `"Success!"` en la consola después de aproximadamente 1000 ms.
  - El estado y el valor de `promise1` serán `resolved` y `"Success!"`.
  - El estado y el valor de `promise2` serán `resolved` y `true`.
- En aproximadamente la mitad de los casos, esto imprimirá inmediatamente `"NOPE!"` en la consola.
  - El estado y el valor de `promise1` serán `rejected` y `Nope!`.
  - El estado y el valor de `promise2` serán `resolved` y `false`.

Es importante entender que, debido a las reglas del ciclo de vida, cuando se llama a `reject`, el `resolve` que llega unos 1000 ms más tarde se ignora en silencio, ya que el estado interno no puede cambiar una vez que se ha rechazado o resuelto.
...
A veces quieres ejecutar código después de que una promesa finalice, sin importar si se resuelve o se rechaza.
...
- En los casos en que `randomNumber` es `4-5`:
  ...
  - Habrá un mensaje en la consola:
    - `Promise completed`
    - _en algunos entornos_ esto producirá un registro `"uncaught rejected promise: Error('Sampling did not result in a sample')"`

Como se muestra arriba, `reject` funciona con un string, y una promesa también puede rechazarse con un `Error`.

<!-- prettier-ignore -->
~~~exercism/note
Si encadenar promesas o el uso general no te queda claro, el [tutorial de MDN][mdn-promises] es un buen recurso para consultar.
